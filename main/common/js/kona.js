...
const BUILD_ID = "kona library __20251211-091317-e3f08b3__";
console.log(BUILD_ID);

(function (global) {
  global.com = global.com || {};
  global.com.idc = global.com.idc || {};
  ...
  const clm = {
    varsTemplate: {
      ...
      ai: {
        active: null,
        config: {
          zapierWebhookUrl: "https://hooks.zapier.com/hooks/catch/25496430/ukmyhjj/",
          s3BaseUrl: "https://idc-clm.s3.us-east-1.amazonaws.com/clm-responses/prod/",
          pollIntervalMs: null,
          maxWaitMs: null,
          tooLongMS: 10 * 1000,
        },
    ...
    init: async function () {
      ...
      //main init tasks -------------------------------------------------
      {
        ...
        //ms log
        now = new Date();
        util.log(`Completed initial sequence - ${now - loadingStarted} ms`);

        //konaReady event
        util.dispatchEvent("konaReady", {
      ...
      //post init tasks -------------------------------------------------
      {
        //read context objects data
        await this.getDataForContextObjects();
        util.dispatchEvent("konaAfterContextObjects", {});

        //ms log
        now = new Date();
        util.log(`After context objects - ${now - loadingStarted} ms`);

        //automatically build interaction summary model and stats (if an actual call)
        if (this.vars.interactionSummary.active) {
          if (this.vars.session.isAnActualCall) {
            //build data model (account for this call)
            await this.interactionSummaryDataModel(clm.vars.metadata.account.id); //build data model for the current account
            ...
            //ms log
            now = new Date();
            util.log(`After interaction summary model - ${now - loadingStarted} ms`);
          }
        }

        //media detection interval
        if (this.vars.options.mediaDetection.reloadAfterAccountSelectionAlert && !this.vars.options.browserMode.active && !this.vars.session.isAnActualCall) {
          this.mediaDetectionIntervalStart();
        }

        //ui after context objects (e.g. media detection / or update item state of related CLM)
    ...
    readSettings: function () {
      ...
      vars.options.debugMode.detailedLog = util.readSetting(com_idc_params, "options.debugMode.detailedLog", "boolean", true);
      ...
      if (com_idc_params.interactionSummary) {
        ...
        //options
        vars.interactionSummary.options.considerSavedCalls = util.readSetting(
          com_idc_params,
          "interactionSummary.options.considerSavedCalls",
          "boolean",
          false
        );
        ...
        vars.interactionSummary.options.accountView.active = util.readSetting(
          com_idc_params,
          "interactionSummary.options.accountView.active",
          "boolean",
          false
        );
        vars.interactionSummary.options.accountView.minRows.previousInteractions = util.readSetting(
          com_idc_params,
          "interactionSummary.options.accountView.minRows.previousInteractions",
          "number",
          1
        );
        vars.interactionSummary.options.accountView.minRows.emails = util.readSetting(
          com_idc_params,
          "interactionSummary.options.accountView.minRows.emails",
          "number",
          1
        );
        vars.interactionSummary.options.accountView.groupViewsForSameSlide = util.readSetting(
          com_idc_params,
          "interactionSummary.options.accountView.groupViewsForSameSlide",
          "boolean",
          false
        );
        vars.interactionSummary.options.allAccountsView.active = util.readSetting(
          com_idc_params,
          "interactionSummary.options.allAccountsView.active",
          "boolean",
          false
        );

        //components
        vars.interactionSummary.components.waitView.id = util.readSetting(com_idc_params, "interactionSummary.components.waitView.id", "string", null);
        ...
        vars.interactionSummary.components.accountView.tab.id = util.readSetting(
          com_idc_params,
          "interactionSummary.components.accountView.tab.id",
          "string",
          null
        );
        vars.interactionSummary.components.allAccountsView.tab.id = util.readSetting(
          com_idc_params,
          "interactionSummary.components.allAccountsView.tab.id",
          "string",
          null
        );
        ...
        vars.interactionSummary.testModel.accounts.min = util.readSetting(com_idc_params, "interactionSummary.testModel.accounts.min", "number", 20);
        vars.interactionSummary.testModel.accounts.max = util.readSetting(com_idc_params, "interactionSummary.testModel.accounts.max", "number", 100);
        ...
        vars.interactionSummary.fields.Email_Activity_vod__c = util.readSetting(
          com_idc_params,
          "interactionSummary.fields.Email_Activity_vod__c",
          "object",
          []
        );
        ...
        vars.interactionSummary.visibility = util.readSetting(
          ...
          "object",
          clm.varsTemplate.interactionSummary.visibility
        );
        ...
        vars.interactionSummary.nextBestContent = util.readSetting(
          ...
          "object",
          clm.varsTemplate.interactionSummary.nextBestContent
        );
      }
      ...
      //ai
      if (com_idc_params.ai) {
        vars.ai.active = util.readSetting(com_idc_params, "ai.active", "boolean", false);
        vars.ai.config.pollIntervalMs = util.readSetting(com_idc_params, "ai.config.pollIntervalSec", "number", 1) * 1000;
        vars.ai.config.maxWaitMs = util.readSetting(com_idc_params, "ai.config.maxWaitSec", "number", 30) * 1000;
        vars.ai.labels = util.readSetting(com_idc_params, "ai.labels", "object", null);
        vars.ai.suggestedQuestions = util.readSetting(com_idc_params, "ai.suggestedQuestions", "object", []);
      }
    },
    setSessionIdentifier: function () {
      return new Promise((resolve) => {
        ...
        util.log("clm.setSessionIdentifier()");
    ...
    setStandardSlidesSequence: function () {
      let vars = this.vars;
      util.log("clm.setStandardSlidesSequence()");
    ...
    setDynamicSlidesSequence: function (activeMode) {
      let vars = this.vars;
      util.log("clm.setDynamicSlidesSequence()");
    ...
    identifyActualSlidesSequence: function () {
      return new Promise((resolve) => {
        (async () => {
          util.log("clm.identifyActualSlidesSequence()");
          ...
          if (activeDynamicPresentationMode) {
            ...
            clm.setDynamicSlidesSequence(activeDynamicPresentationMode);
          } else {
            //it is a standard presentation
            clm.setStandardSlidesSequence();
          }
    ...
    addSlideToNavHistory: async function (slideId) {
      return new Promise((resolve) => {
        (async () => {
          ...
          if (slide) {
            util.log("clm.addSlideToNavHistory()");
    ...
    getDataForContextObjects: function () {
      ...
      let detailedLog = this.vars.options.debugMode.detailedLog;

      //only for actual calls
      if (this.vars.options.browserMode.active) {
        this.generateContextObjectsTestData();
        return;
      }

      //used for developing/debugging, will disable cache even if available
      const forceDisabledCache = false;

      return new Promise((resolve) => {
        (async () => {
          /*get account(s) metadata ------------------------------------------------------------------------*/
          {
            //check if data was already cached in session data, otherwise query
            if (
              ((this.vars.session.isAnActualCall && sessionData.metadata.call.cached) ||
                (!this.vars.session.isAnActualCall && sessionData.metadata.accounts.cached)) &&
              !forceDisabledCache
            ) {
              //data was cached in session data >> use it
              if (this.vars.session.isAnActualCall) {
                this.vars.metadata.account = JSON.parse(JSON.stringify(sessionData.metadata.call.account));
              } else {
                this.vars.metadata.allAccounts = JSON.parse(JSON.stringify(sessionData.metadata.accounts.allAccounts));
              }

              if (detailedLog) util.log("clm.getDataForContextObjects() >> using cached account data");
            } else {
              ...
              if (detailedLog) util.log("clm.getDataForContextObjects() >> read account(s)");
            }
          }
          ...
          /*get media-related metadata (presentation, key message) -----------------------------------------*/
          {
            if (sessionData.metadata.media.cached && !forceDisabledCache) {
              ...
              if (detailedLog) util.log("clm.getDataForContextObjects() >> using cached presentation data");
                ...
                this.vars.emailCart.templates
                  ...
                  .forEach((item) => {
                    if (item.vaultId) {
                      ...
                      util.log(`clm.getDataForContextObjects: missing Vault Doc ID for emailCart template ${item.id}`, "error");
                    }
                  });
                this.vars.emailCart.fragments
                  ...
                  .forEach((item) => {
                    if (item.vaultId) {
                      ...
                      util.log(`clm.getDataForContextObjects: missing Vault Doc ID for emailCart fragment ${item.id}`, "error");
                    }
                  });
                    ...
                    if (template.vaultId) {
                      ...
                      template.fragments.forEach((fragment) => {
                        if (fragment.vaultId) {
                          ...
                          util.log(`clm.getDataForContextObjects: missing Vault Doc ID for interactionSummary fragment ${fragment.id}`, "error");
                        }
                      });
                    } else {
                      util.log(`clm.getDataForContextObjects: missing Vault Doc ID for interactionSummary template ${template.id}`, "error");
                    }
                  });
                }
              }
                  ...
                  await new Promise((resolve) => {
                    com.veeva.clm.getApprovedDocument(this.vars.emailCart.vaultURL, item.vaultId, (data) => {
                      if (data.success && data.Approved_Document_vod__c) {
                        ...
                        util.log(
                          `clm.getDataForContextObjects: could not retrieve CRM ID for ${item.id} (${item.group}): ${item.vaultId} + ${this.vars.emailCart.vaultURL}`,
                          "error"
                        );
            ...
            if (sessionData.relatedCLM.cached && !forceDisabledCache) {
              for (let relatedItem of this.vars.relatedCLM) {
                ...
                if (sessionItem) {
                  ...
                  util.log(`clm.getDataForContextObjects: related CLM / no approved or staged records found for ${relatedItem.id}`, "error");
                }
              }
              if (detailedLog) util.log("clm.getDataForContextObjects() >> using cached related CLM data");
            } else {
              let needToUpdateSessionData = false;
              for (let relatedItem of this.vars.relatedCLM) {
                let idsToCheck = [
              ...
              if (needToUpdateSessionData) {
                ...
                await this.updateSessionData();
                if (detailedLog) util.log("clm.getDataForContextObjects() >> read related CLM");
              }
            }
          }
    ...
    generateContextObjectsTestData: function () {
      let vars = this.vars;

      const minAccounts = this.vars.interactionSummary.testModel.accounts.min;
      const maxAccounts = this.vars.interactionSummary.testModel.accounts.max;

      //account(s) just one if actual call or multiple if media --------------------------------------------------------
      if (this.vars.session.isAnActualCall) {
        //call: one account (use simulate object from config if available)
        if (this.vars.options.browserMode.simulate.objects.Account) {
          ...
          this.vars.metadata.account.recordTypeName = this.vars.options.browserMode.simulate.objects.Account.RecordTypeName;
        } else {
        ...
        let hcpNames, hcoNames;
        {
          ...
          hcoNames = [
            ...
            "Northshore Specialty Care",
            "Eastgate Family Medicine",
            "Regional Cancer Institute",
            "Lakeside Orthopedic Center",
            "Downtown Urgent Care",
            "Mountainview Cardiology",
            "Coastal Emergency Hospital",
            "Pine Ridge Surgery Center",
            "Unity Health Partners",
            "Springfield General Hospital",
            "Harbor Medical Associates",
            "Crossroads Family Health",
            "Summit Rehabilitation Center",
            "Greenfield Women's Health",
          ];
        }

        //account template
        let accountTemplate = this.vars.metadata.allAccounts.splice(0)[0];

        for (let i = 0; i < accountsCount; i++) {
          //id: generate consecutive numbers with leading zeros
          let consecutiveNumber = i + 1;
          let accountId = "00000000000000000".substring(0, 17 - consecutiveNumber.toString().length) + consecutiveNumber;

          //recordTypeName: select record type, 50% of the times Professional, otherwise HCO
          let recordTypeRandom = Math.random();
          let accountRecordType =
            recordTypeRandom < 0.5 ? "Professional" : recordTypeRandom < 0.75 ? "Hospital" : recordTypeRandom < 0.9 ? "Clinic" : "Practice";

          //name: select random name from list depending on record type
          let accountName = "";
          if (accountRecordType == "Professional") {
            //HCP
            if (hcpNames.length > 0) {
              let nameIndex = Math.floor(Math.random() * hcpNames.length);
              accountName = hcpNames[nameIndex];
              hcpNames.splice(nameIndex, 1); //remove name to avoid duplicates
            } else {
              accountName = "HCP account " + i;
            }
          } else {
            //HCO
            if (hcoNames.length > 0) {
              let nameIndex = Math.floor(Math.random() * hcoNames.length);
              accountName = hcoNames[nameIndex];
              hcoNames.splice(nameIndex, 1); //remove name to avoid duplicates
            } else {
              accountName = "HCO account " + i;
            }
          }

          //salutation: Dr. 30% of the times for HCP, empty for HCO
          let accountSalutation = "";
          if (accountRecordType == "Professional") {
            let salutationRandom = Math.random();
            if (salutationRandom < 0.3) {
              accountSalutation = "Dr.";
            }
          }

          //add record
          let accountRecord = JSON.parse(JSON.stringify(accountTemplate));
      ...
      if (this.vars.emailCart.active) {
        ...
        if (this.vars.emailCart.active) {
          if (this.vars.emailCart.templates.length == 0) {
            util.log("clm.generateInteractionSummaryTestData: email cart is active but no templates found", "error");
          }
          this.vars.emailCart.templates.forEach((template) => {
            if (template.vaultId) {
              ...
              util.log(`clm.generateInteractionSummaryTestData: missing Vault Doc ID for template ${template.id}`, "error");
            }
          });
          this.vars.emailCart.fragments.forEach((fragment) => {
            if (fragment.vaultId) {
              ...
              util.log(`clm.generateInteractionSummaryTestData: missing Vault Doc ID for fragment ${fragment.id}`, "error");
            }
          });
        }
        this.vars.interactionSummary.nonEmailCartItems.templates.forEach((template) => {
    ...
    loadSessionData: function () {
      ...
      return new Promise((resolve) => {
        ...
        try {
          ...
          util.log(`com.idc.loadSessionData: ${err}`, "error");
        }
    ...
    gotoSlide: async function (slideId) {
      ...
      if (slideId && slideId.endsWith("..")) {
        let slideFound = clm.findSlide(slideId.replace("..", ""), true);
        if (slideFound) {
      ...
      if (clm.vars.navigation.allAvaliableSlides.findIndex((slide) => slide === slideId) < 0) return;
    ...
    handleSwipeNavigation: function (pSwipe) {
      let vars = clm.vars;
      let sessionData = clm.sessionData;
      ...
      let isAModalBeingDisplayed = util.getElementAttribute(document.querySelector("body"), "data-modal-state") == "active";
      let treatStandaloneModalsAsMainSlides = vars.navigation.dynamicPresentation.treatStandaloneModalsAsMainSlides;
      ...
      if (isStandalone && isStandaloneGroupActive) {
        ...
        switch (pSwipe.direction) {
          case "left":
            if (vars.navigation.overWrite.nextSlide) clm.goNextSlide();
            break;
          case "right":
            if (vars.navigation.overWrite.prevSlide) clm.goPrevSlide();
            break;
        }
      }
    },
    ...
    externalFunction: function () {
      return new Promise((resolve) => {
        ...
        if (slidesSequence.length == 0) {
          util.log(`clm.externalFunction: no slides found in ${functionName}()} ${slidesSequenceInput}`, "error");
        }
        ...
        (async () => {
          ...
          if (selectedCallflow) {
            ...
            if (!slidesSequence || slidesSequence.length == 0) {
              util.log(`clm.callflows: unable to retrieve slides for callflow ${selectedCallflow}`, "error");
            }
          }
    ...
    setCallflow: function (callflowName) {
      ...
      if (!this.vars.options.dynamicPresentation.source.callflows.active) {
        util.log(`clm.setCallflow: callflows source is inactive`, "error");
        return;
      }
      ...
      if (this.vars.options.dynamicPresentation.source.callflows.flows.find((callflow) => callflow.name == callflowName)) {
        ...
        util.log(`clm.setCallflow: invalid callflow name ${callflowName}`, "error");
      }
      ...
      if (selectedCallflow) {
        ...
        if (slidesSequence.length == 0) {
          util.log(`clm.setCallflow: unable to retrieve slides for callflow ${selectedCallflow}`, "error");
        }
      }
      ...
      if (slidesSequence.length > 0) {
        ...
        ui.dynamicPresentation.setMenu(this.vars.navigation.actualSlidesSequence);
        ui.dynamicPresentation.setAlertPopup();
        ui.core.link.flagNonWorkingLinks();
    ...
    unSetCallflow: function () {
      ...
      ui.dynamicPresentation.unSetMenu();
      ui.dynamicPresentation.unSetAlertPopup();
      ui.core.link.unFlagNonWorkingLinks();
    ...
    validateStandaloneGroup: function (group, slideId) {
      ...
      if (this.vars.navigation.dynamicPresentation.active) {
        util.log("clm.validateStandaloneGroup: dynamic presentation / unable to activate standalone group", "error");
        return false;
      }
      ...
      if (!groupObj) {
        util.log(`clm.validateStandaloneGroup: group ${group} not found`, "error");
        groupIsValid = false;
      }
      ...
      if (slideId) {
        if (!groupObj.slides.includes(slideId)) {
          util.log(`clm.validateStandaloneGroup: slide ${slideId} not found in group ${group}`, "error");
          groupIsValid = false;
        }
      }
    ...
    interactionSummaryDataModel: function (pAccountId) {
      let vars = clm.vars;
      let sessionData = clm.sessionData;
      let detailedLog = vars.options.debugMode.detailedLog;

      // disable cache flag for testing purposes
      const forceDisabledCache = false;

      return new Promise((resolve) => {
        (async () => {
          /*use catched data if available or obtain data ----------------------------------------------------*/
          if (pAccountId && sessionData.interactionSummary.cached && !forceDisabledCache) {
            /*used cached data for current account ----------------------------------------------------------*/
            vars.interactionSummary.output.currentAccount = JSON.parse(JSON.stringify(sessionData.interactionSummary.output.currentAccount));
            /*used cached data for names ----------------------------------------------------------*/
            vars.interactionSummary.output.contentItems = JSON.parse(JSON.stringify(sessionData.interactionSummary.output.contentItems));

            if (detailedLog) util.log("clm.getDataForContextObjects() >> using cached interaction summary data for current account");

            resolve();
          } else {
            /*obtain data ------------------------------------------------------------------------------------*/
            {
              /*if no pAccountId and need to read all accounts, restrict params for performance ---------------*/
              if (!pAccountId) {
                if (vars.interactionSummary.options.considerCallsWithOtherPresentations) {
                  vars.interactionSummary.options.considerCallsWithOtherPresentations = false;
                  if (detailedLog) util.log("clm.getDataForContextObjects() >> disabled considerCallsWithOtherPresentations for non-actual call");
                }
                if (vars.interactionSummary.options.considerSavedCalls) {
                  vars.interactionSummary.options.considerSavedCalls = false;
                  if (detailedLog) util.log("clm.getDataForContextObjects() >> disabled considerSavedCalls for non-actual call");
                }
                if (vars.interactionSummary.options.considerEmailsWithOtherTemplates) {
                  vars.interactionSummary.options.considerEmailsWithOtherTemplates = false;
                  if (detailedLog) util.log("clm.getDataForContextObjects() >> disabled considerEmailsWithOtherTemplates for non-actual call");
                }
              }

              //if browserMode, generate test data; otherwise, retrieve from CRM
              if (vars.options.browserMode.active) {
                //test data: browser mode and simulated call
                clm.generateInteractionSummaryTestData();
              } else {
                //real data: CRM mode or actual call in browser mode

                //obtain Key Messages IDs of current presentation
                await new Promise((resolve) => {
                  let whereClause = vars.slides
                    .map((slide) => {
                  ...
                  com.veeva.clm.queryRecord("Key_Message_vod__c", ["ID", "Media_File_Name_vod__c"], whereClause, [], null, (data) => {
                    if (data.success) {
                      ...
                      util.log(`clm.getDataForContextObjects: failed to retrieve Key_Message_vod__c IDs ${data.message}`, "error");
                      resolve();
                    }
                  });
                });
                ...
                if (vars.relatedCLM.length > 0) {
                  for (let relatedItem of vars.relatedCLM) {
                        ...
                        com.veeva.clm.queryRecord(
                          ...
                          (data) => {
                            if (data.success && data.Clm_Presentation_vod__c.length > 0) {
                              ...
                              util.log(
                                `clm.getDataForContextObjects: failed to retrieve Presentation_vod__c for relatedCLM ${relatedItem.id}, vaultDocId ${relatedItem.vaultExternalID.presentation}`,
                                "error"
                              );
                            }
                            resolve();
                          ...
                          com.veeva.clm.queryRecord(
                            ...
                            (data) => {
                              if (data.success && data.Clm_Presentation_Slide_vod__c.length > 0) {
                                ...
                                util.log(
                                  `clm.getDataForContextObjects: failed to retrieve Clm_Presentation_Slide_vod__c for relatedCLM ${relatedItem.id}, vaultDocId ${relatedItem.vaultExternalID.presentation}`,
                                  "error"
                                );
                              }
                              resolve();
                          ...
                          let thisCLMPresentation_KeyMessages = await new Promise((resolve) => {
                            ...
                            com.veeva.clm.queryRecord("Key_Message_vod__c", ["ID", "Media_File_Name_vod__c"], whereClause, [], null, (data) => {
                              if (data.success && data.Key_Message_vod__c.length > 0) {
                                ...
                                util.log(
                                  `clm.getDataForContextObjects: failed to retrieve Key_Message_vod__c for relatedCLM ${relatedItem.id}, vaultDocId ${relatedItem.vaultExternalID.presentation}`,
                                  "error"
                                );
                              }
                              resolve();
                  ...
                  vars.slides
                    ...
                    .forEach((slide) => {
                      keyMessagesMatrix.push({
                        Presentation: vars.interactionSummary.labels.accountView.thisPresentation,
                ...
                let callKeyMessageRecords = [];
                {
                  ...
                  let whereClause;
                  {
                    ...
                    if (vars.interactionSummary.options.considerCallsWithOtherPresentations) {
                      if (pAccountId) {
                        whereClause = `Account_vod__c = "${vars.metadata.account.id}"`;
                      } else {
                        whereClause = null;
                      }
                    } else {
                      if (pAccountId) {
                        whereClause = `Account_vod__c = "${vars.metadata.account.id}" AND (${keyMessagesWhereClause})`;
                      } else {
                        whereClause = `${keyMessagesWhereClause}`;
                      }
                    }
                  }
                  ...
                  callKeyMessageRecords = await new Promise((resolve) => {
                    com.veeva.clm.queryRecord(
                      "Call2_Key_Message_vod__c",
                      vars.interactionSummary.fields.Call2_Key_Message_vod__c,
                      whereClause,
                      [],
                      null,
                      (data) => {
                        if (data.success) {
                          ...
                          util.log(`clm.getDataForContextObjects: failed to retrieve Call2_Key_Message_vod__c records ${data.message}`, "error");
                          resolve([]);
                        }
                      }
                    );
                  ...
                  if (callKeyMessageRecords) {
                    vars.interactionSummary.input.Call2_Key_Message_vod__c = callKeyMessageRecords;
                  }
                }
                ...
                let callRecords = [];
                {
                  if (vars.interactionSummary.input.Call2_Key_Message_vod__c.length > 0) {
                    //unique call IDs from call key message records
                    let uniqueCallIds = [
                      ...new Set(vars.interactionSummary.input.Call2_Key_Message_vod__c.map((callKeyMessage) => callKeyMessage.Call2_vod__c)),
                    ];
                    ...
                    for (const callIdBatch of callIdBatches) {
                      ...
                      const batchCallRecords = await new Promise((resolve) => {
                        com.veeva.clm.queryRecord("Call2_vod__c", vars.interactionSummary.fields.Call2_vod__c, whereClause, [], null, (data) => {
                          if (data.success) {
                            resolve(data.Call2_vod__c);
                          } else {
                            util.log(`clm.getDataForContextObjects: failed to retrieve Call2_vod__c records for batch ${data.message}`, "error");
                            resolve([]);
                          }
                        });
                      });
                    ...
                    if (callRecords) {
                      vars.interactionSummary.input.Call2_vod__c = callRecords;
                    }
                  }
                }
                ...
                if (vars.emailCart.active) {
                  ...
                  //query records
                  if (vars.emailCart.templates && vars.interactionSummary.input.Account && vars.interactionSummary.input.Account.length > 0) {
                    // all templates and where clause
                    let allTemplates = vars.emailCart.templates.concat(vars.interactionSummary.nonEmailCartItems.templates);
                    ...
                    if (vars.interactionSummary.options.considerEmailsWithOtherTemplates) {
                      ...
                      if (
                        ...
                        util.log(`clm.getDataForContextObjects: no templates with crmId`, "error");
                        resolve([]);
                        return;
                      }
                    ...
                    const accountBatches = util.chunkArray(vars.interactionSummary.input.Account, 10);
                    ...
                    for (const accountBatch of accountBatches) {
                      const batchSentEmailRecords = await new Promise((resolve) => {
                        ...
                        com.veeva.clm.queryRecord("Sent_Email_vod__c", vars.interactionSummary.fields.Sent_Email_vod__c, whereClause, [], null, (data) => {
                          if (data.success) {
                            resolve(data.Sent_Email_vod__c);
                          } else {
                            util.log(`clm.getDataForContextObjects: failed to retrieve Sent_Email_vod__c records for batch ${data.message}`, "error");
                            resolve([]);
                          }
                        });
                      });
                  ...
                  //add to interaction summary input
                  if (sentEmailRecords) {
                    vars.interactionSummary.input.Sent_Email_vod__c = sentEmailRecords;
                  }
                  ...
                  if (sentEmailRecords && vars.interactionSummary.options.considerEmailsWithOtherTemplates) {
                    ...
                    sentEmailRecords.forEach((sentEmail) => {
                      ...
                      if (templateIndex < 0) {
                        ...
                        vars.interactionSummary.nonEmailCartItems.templates.push(newTemplate);

                        templateIndex = vars.interactionSummary.nonEmailCartItems.templates.length - 1;
                      }
                      ...
                      if (sentEmail.Email_Fragments_vod__c && sentEmail.Email_Fragments_vod__c.length > 0) {
                        let thisTemplate = vars.interactionSummary.nonEmailCartItems.templates[templateIndex];
                    ...
                    if (allCRMIDs.length > 0) {
                      // Split CRM IDs into batches of 500
                      const crmIdBatches = util.chunkArray(allCRMIDs, 500);
                          ...
                          com.veeva.clm.queryRecord(
                            ...
                            (data) => {
                              if (data.success) {
                                ...
                                util.log(
                                  `clm.getDataForContextObjects: failed to retrieve Approved_Document_vod__c records for batch ${data.message}`,
                                  "error"
                                );
                                resolve([]);
                              }
                    ...
                    vars.interactionSummary.nonEmailCartItems.templates = uniqueTemplates;
                  }
                }
                ...
                if (vars.emailCart.active) {
                  ...
                  if (vars.interactionSummary.input.Sent_Email_vod__c && vars.interactionSummary.input.Sent_Email_vod__c.length > 0) {
                    // Split sent emails into batches (using 500 as a reasonable batch size for email activity records)
                    const sentEmailBatches = util.chunkArray(vars.interactionSummary.input.Sent_Email_vod__c, 500);
                    ...
                    for (const sentEmailBatch of sentEmailBatches) {
                      const batchEmailActivityRecords = await new Promise((resolve) => {
                        ...
                        com.veeva.clm.queryRecord(
                          "Email_Activity_vod__c",
                          vars.interactionSummary.fields.Email_Activity_vod__c,
                          whereClause,
                          [],
                          null,
                          (data) => {
                            if (data.success) {
                              ...
                              util.log(`clm.getDataForContextObjects: failed to retrieve Email_Activity_vod__c records for batch ${data.message}`, "error");
                              resolve([]);
                            }
                          }
                        );
                  ...
                  if (emailActivityRecords) {
                    vars.interactionSummary.input.Email_Activity_vod__c = emailActivityRecords;
                  }
                }

                //Next best content CLM
                if (vars.interactionSummary.nextBestContent.clm.active) {
                  let sourceFnc = vars.interactionSummary.nextBestContent.clm.source.call;
                  if (sourceFnc !== "undefined" && sourceFnc !== "" && typeof window[sourceFnc] === "function") {
                    let nextBestContentCLM = await window[sourceFnc]();
                    if (Array.isArray(nextBestContentCLM) && nextBestContentCLM.length > 0) {
                      vars.interactionSummary.input.Next_Best_Content_CLM = nextBestContentCLM;
                    }
                  }
                }
              }
            ...
            /*build model and store (vars and session vars if necessary) -------------------------------------*/
            {
              //initialize input accounts array (one account or all accounts)
              ...
              //store names for slides, emails and related CLM
              {
                //slides
                let slideNameTemplate = JSON.parse(JSON.stringify(vars.interactionSummary.output.templates.contentItemTemplates.slide));
                vars.slides.forEach((slide) => {
                  let slideNameRecord = JSON.parse(JSON.stringify(slideNameTemplate));
                  slideNameRecord.id = slide.id;
                  slideNameRecord.title = slide.description;
                  vars.interactionSummary.output.contentItems.slides.push(slideNameRecord);
                });

                //emails
                let emailNameTemplate = JSON.parse(JSON.stringify(vars.interactionSummary.output.templates.contentItemTemplates.email));
                let fragmentNameTemplate;

                vars.emailCart.templates.forEach((template) => {
                  let emailNameRecord = JSON.parse(JSON.stringify(emailNameTemplate));

                  //populate id and title
                  emailNameRecord.id = template.id;
                  emailNameRecord.title = template.title;
                  emailNameRecord.thumbnail = template.thumb;

                  //fragments
                  if (vars.emailCart.fragments.length > 0) {
                    fragmentNameTemplate = emailNameRecord.fragments.splice(0)[0];
                    vars.emailCart.fragments.forEach((fragment) => {
                      let fragmentNameRecord = JSON.parse(JSON.stringify(fragmentNameTemplate));

                      //populate id and title
                      fragmentNameRecord.id = fragment.id;
                      fragmentNameRecord.title = fragment.title;

                      //add to array
                      emailNameRecord.fragments.push(fragmentNameRecord);
                    });
                  }

                  //add to array
                  vars.interactionSummary.output.contentItems.emails.push(emailNameRecord);
                });
                ...
                //related CLM
                let relatedCLMNameTemplate = JSON.parse(JSON.stringify(vars.interactionSummary.output.templates.contentItemTemplates.relatedCLM));
                vars.relatedCLM.forEach((relatedCLM) => {
                  let relatedCLMNameRecord = JSON.parse(JSON.stringify(relatedCLMNameTemplate));

                  //populate id and name
                  relatedCLMNameRecord.id = relatedCLM.id;
                  relatedCLMNameRecord.name = relatedCLM.name;
                  relatedCLMNameRecord.thumb = relatedCLM.thumb;

                  //add to array
                  vars.interactionSummary.output.contentItems.relatedCLM.push(relatedCLMNameRecord);
                });
              }

              //for each account
              accountsToProcess.forEach((account) => {
                //outputRecord template
                let outputRecord = JSON.parse(JSON.stringify(vars.interactionSummary.output.templates.accountTemplate));

                //account --------------------------------------------
                outputRecord.account = account;
                ...
                //timeline -------------------------------------------
                {
                  ...
                  vars.interactionSummary.input.Call2_vod__c.filter((call) => call.Account_vod__c == outputRecord.account.id) //filter by account
                    .forEach((call) => {
                      ...
                      if (vars.interactionSummary.options.accountView.groupViewsForSameSlide) {
                        let newSlidesArr = [];
                      ...
                      //add to timeline
                      outputRecord.timeline.push(record);
                    });

                  //email records
                  vars.interactionSummary.input.Sent_Email_vod__c.filter((sentEmail) => sentEmail.Account_vod__c == outputRecord.account.id) //filter by account
                    .forEach((sentEmail) => {
                      ...
                      let template, templateType;
                      {
                        ...
                        if (!template) {
                          template = vars.interactionSummary.nonEmailCartItems.templates.find(
                            (template) => template.crmId == sentEmail.Approved_Email_Template_vod__c
                          );
                  ...
                  outputRecord.timeline.sort((a, b) => {
                ...
                //slides ---------------------------------------------
                {
                  const slideTemplate = outputRecord.slides.splice(0)[0];
                  ...
                  vars.slides.forEach((slide) => {
                    ...
                    let callKeyMessageRecords = vars.interactionSummary.input.Call2_Key_Message_vod__c.filter((callKeyMessage) => {
                      //filter by account
                      ...
                      .filter((callKeyMessage) => vars.interactionSummary.input.Call2_vod__c.find((call) => call.ID == callKeyMessage.Call2_vod__c)) // filter by existing calls
                  ...
                  if (vars.interactionSummary.nextBestContent.clm.active) {
                    vars.interactionSummary.input.Next_Best_Content_CLM.forEach((item) => {
                      let slideIndex = outputRecord.slides.findIndex(function (slide) {
                        if (item.zipName) {
                      ...
                      if (slideIndex >= 0) {
                        outputRecord.slides[slideIndex].nextBestContent.recommended = true;
                        if (Number.isInteger(item.order)) {
                          outputRecord.slides[slideIndex].nextBestContent.order = item.order;
                        }
                      }
                    });
                  }
                ...
                //emails ---------------------------------------------
                {
                  const emailTemplate = outputRecord.emails.splice(0)[0];
                  ...
                  vars.emailCart.templates.forEach((template) => {
                    ...
                    //add to array
                    outputRecord.emails.push(record);
                  });
                  ...
                  outputRecord.emails.forEach((email) => {
                    ...
                    let sentEmails = vars.interactionSummary.input.Sent_Email_vod__c.filter((sentEmail) => sentEmail.Account_vod__c == outputRecord.account.id) //filter by account
                ...
                //relatedCLM -----------------------------------------
                {
                  const relatedCLMTemplate = outputRecord.relatedCLM.splice(0)[0];
                  ...
                  vars.relatedCLM.forEach((relatedCLM) => {
                    ...
                    outputRecord.relatedCLM.push(newRecord);
                  });

                  //populate data from calls and call key message records
                  vars.interactionSummary.input.Call2_vod__c.filter((call) => {
                    ...
                    .forEach((call) => {
                      ...
                      presentations.forEach((presentation) => {
                        ...
                        let recordIndex = outputRecord.relatedCLM.findIndex((item) => item.name == presentation.name);
                  ...
                  outputRecord.relatedCLM.forEach((relatedCLM) => {
                ...
                //this presentation ----------------------------------
                {
                  ...
                  outputRecord.timeline
                    .filter((timelineRecord) => {
                      return timelineRecord.type == "call" && timelineRecord.call.presentations.findIndex((presentation) => presentation.thisPresentation) > -1;
                    })
                    .forEach((timelineRecord) => {
                      callsCount++;
                      let recordDate = new Date(timelineRecord.date); //yyyy-mm-dd
                      if (!lastActivityDate || recordDate > lastActivityDate) {
                        lastActivityDate = timelineRecord.date;
                      }
                    });
                  outputRecord.thisPresentation.calls = callsCount;
                  outputRecord.thisPresentation.lastActivityDate = lastActivityDate;
                }

                //flags ----------------------------------------------
                {
                  if (!outputRecord.account.flags) outputRecord.account.flags = {};
                  
                  //visiting this week
                  let thisWeeksMonday = new Date();
                  thisWeeksMonday.setDate(thisWeeksMonday.getDate() - thisWeeksMonday.getDay() + 1); //set to this week's Monday
                  thisWeeksMonday.setHours(0, 0, 0, 0);
                  let thisWeeksSunday = new Date(thisWeeksMonday);
                  thisWeeksSunday.setDate(thisWeeksSunday.getDate() + 6); //set to this week's Sunday
                  thisWeeksSunday.setHours(23, 59, 59, 999);
                  vars.interactionSummary.input.Call2_vod__c.filter((call) => call.Account_vod__c == outputRecord.account.id) //filter by account
                    .forEach((call) => {
                      let callDate = new Date(call.Call_Datetime_vod__c);
                      if (callDate >= thisWeeksMonday && callDate <= thisWeeksSunday) {
                ...
                // add to all accounts
                vars.interactionSummary.output.allAccounts.push(outputRecord);
              });
            }

            /*activity by content element --------------------------------------------------------------------*/
            {
              vars.interactionSummary.output.allAccounts.forEach((accountRecord) => {
                let clmActivityTemplate = JSON.parse(JSON.stringify(vars.interactionSummary.output.templates.contentItemTemplates.clmAccountActivity));
                let emailActivityTemplate = JSON.parse(JSON.stringify(vars.interactionSummary.output.templates.contentItemTemplates.emailAccountActivity));

                //this presentation
                {
                  let clmActivityRecord = JSON.parse(JSON.stringify(clmActivityTemplate));

                  //account details
                  clmActivityRecord.id = accountRecord.account.id;
                  clmActivityRecord.name = accountRecord.account.name;

                  //flags
                  clmActivityRecord.flags.visitingThisWeek = accountRecord.account.flags.visitingThisWeek || false;

                  //activity
                  if (!accountRecord.noActivity) {
                    if (accountRecord.thisPresentation.calls > 0) {
                      clmActivityRecord.flags.sawThisMaterial = true;
                    } else {
                      clmActivityRecord.flags.sawThisMaterial = false;
                    }
                    clmActivityRecord.calls = accountRecord.thisPresentation.calls;
                    clmActivityRecord.lastActivity = accountRecord.thisPresentation.lastActivityDate;
                  } else {
                    clmActivityRecord.flags.sawThisMaterial = false;
                  }

                  //add to array
                  vars.interactionSummary.output.contentItems.thisPresentation.accountsActivity.push(clmActivityRecord);
                }

                //relatated CLM
                {
                  accountRecord.relatedCLM.forEach((relatedCLM) => {
                    let clmActivityRecord = JSON.parse(JSON.stringify(clmActivityTemplate));
                    ...
                    if (!accountRecord.noActivity) {
                      ...
                      clmActivityRecord.calls = relatedCLM.overall.timesDisplayed;
                      clmActivityRecord.lastActivity = relatedCLM.mostRecentCall.date;
                    } else {
                      clmActivityRecord.flags.sawThisMaterial = false;
                    }

                    //add to array (find relatedCLM content item before)
                    let contentItemIndex = vars.interactionSummary.output.contentItems.relatedCLM.findIndex((item) => item.id == relatedCLM.id);
                    if (contentItemIndex >= 0) {
                      vars.interactionSummary.output.contentItems.relatedCLM[contentItemIndex].accountsActivity.push(clmActivityRecord);
                    }
                  });
                }
                ...
                //emails
                {
                  accountRecord.emails.forEach((email) => {
                    let emailActivityRecord = JSON.parse(JSON.stringify(emailActivityTemplate));

                    //account details
                    emailActivityRecord.id = accountRecord.account.id;
                    emailActivityRecord.name = accountRecord.account.name;

                    //flags
                    emailActivityRecord.flags.visitingThisWeek = accountRecord.account.flags.visitingThisWeek || false;

                    //activity
                    if (!accountRecord.noActivity) {
                      emailActivityRecord.sent = email.overall.sent;
                      emailActivityRecord.opens = email.overall.opens;
                    } else {

                    }

                    //add to array (find relatedCLM content item before)
                    let contentItemIndex = vars.interactionSummary.output.contentItems.emails.findIndex((item) => item.id == email.id);
                    if (contentItemIndex >= 0) {
                      vars.interactionSummary.output.contentItems.emails[contentItemIndex].accountsActivity.push(emailActivityRecord);
                    }
                  });
                }
              });
            ...
            /*remove unnecesary keys for accounts with no activity -------------------------------------------*/
            {
              vars.interactionSummary.output.allAccounts.forEach((accountRecord) => {
                let hasActivity = false;
                if (accountRecord.timeline.length > 0) hasActivity = true;

                if (!hasActivity) {
                  // remove unnecesary keys
                  delete accountRecord.timeline;
                  delete accountRecord.slides;
                  delete accountRecord.emails;
                  delete accountRecord.relatedCLM;
                  delete accountRecord.thisPresentation;

                  //set no activity flag
                  accountRecord.noActivity = true;
                } else {
                  //remove title from slides
                  accountRecord.slides.forEach((slide) => {
                    delete slide.title;
                  });
                  //remove title and thumb from emails
                  accountRecord.emails.forEach((email) => {
                    delete email.title;
                    delete email.thumbnail;
                  });
                  //remove name and thumb from relatedCLM
                  accountRecord.relatedCLM.forEach((relatedCLM) => {
                    delete relatedCLM.name;
                    delete relatedCLM.thumb;
                  });
                }
              });
            }

            /*call mode: assign currentAccountRecord ---------------------------------------------------------*/
            {
              //set current account output in vars and store in session data
              if (pAccountId) {
                //find record
                let currentAccountRecord = vars.interactionSummary.output.allAccounts.find((record) => record.account.id == pAccountId);

                //set current account in vars
                if (currentAccountRecord) {
                  vars.interactionSummary.output.currentAccount = currentAccountRecord;
                } else {
                  vars.interactionSummary.output.currentAccount = null;
                }

                //store in session data
                sessionData.interactionSummary.output.currentAccount = vars.interactionSummary.output.currentAccount;
                sessionData.interactionSummary.output.contentItems = vars.interactionSummary.output.contentItems;
                sessionData.interactionSummary.cached = true;
                clm.updateSessionData();
              }
            }
          }
          ...
          //log
          util.log(`clm.interactionSummaryDataModel() - (${pAccountId ? vars.metadata.account.id + " account)" : "all accounts)"}`);

          resolve();
        })();
      });
    },
    generateInteractionSummaryTestData: function () {
      let vars = this.vars;
      ...
      let pseudoKeyMessagesMatrix = [];
      {
        ...
        this.vars.slides.forEach((slide) => {
          ...
          pseudoKeyMessagesMatrix.push({
            Presentation: vars.interactionSummary.labels.accountView.thisPresentation,
            ID: idString,
            Media_File_Name_vod__c: vars.interactionSummary.labels.accountView.thisPresentation.replaceAll(" ", "") + "_" + slide.id + ".zip",
            ThisPresentation: true,
      ...
      let accounts = [];
      if (this.vars.session.isAnActualCall) {
        accounts.push(this.vars.metadata.account);
      } else {
        accounts = this.vars.metadata.allAccounts;
      }

      //simulation: do not generate data for 50% of the accounts
      accounts.forEach((account) => {
        if (Math.random() < 0.5) {
          account.doNotGenerateTestData = true;
        }
      });
      ...
      accounts.forEach((account) => {
        delete account.doNotGenerateTestData;
      });
    ...
    emailCartStats: function () {
      ...
      if (!vars.interactionSummary.output.currentAccount) return;
      if (vars.interactionSummary.output.currentAccount.noActivity) return;

      vars.emailCart.templates.forEach((template) => {
        //template stats
        let templateInInteractionSummary = vars.interactionSummary.output.currentAccount.emails.find((email) => email.crmId == template.crmId);
    ...
    /*media detection interval---------------------------------*/
    mediaDetectionIntervalStart: function () {
      let vars = this.vars;
      let detailedLog = vars.options.debugMode.detailedLog;

      vars.mediaDetection.isRunning = true;

      vars.mediaDetection.interval = setInterval(() => {
          ...
          return new Promise((resolve, reject) => {
            ...
            com.veeva.clm.getDataForCurrentObject("Call", "ID", (data) => {
              if (data.success) {
                ...
                //compare with session call id and if different, alert and reload
                if (newCallID != clm.vars.session.callId) {
                  window.alert(clm.vars.options.mediaDetection.labels.reloadAfterAccountSelectionMessage);
    ...
    mediaDetectionIntervalStop: function () {
      let vars = this.vars;

      vars.mediaDetection.isRunning = false;

      clearInterval(vars.mediaDetection.interval);
    },
    ...
    loadCommonHTML: function () {
      ...
      this.vars.commonHTML.elements.forEach((commonElementId) => {
        if (processedItems.includes(commonElementId)) {
          util.log(`clm.loadCommonHTML: duplicated id "${commonElementId}"`, "error");
          return;
        } else {
        ...
        if (!commonElement && elementPlaceholder) {
          util.log(
            `clm.loadCommonHTML: ${commonElementId} not found in localStorage. Please refresh navigate to common/index.html to refresh common elements`,
            "error"
          );
        }
        if (commonElement && elementPlaceholder) {
    ...
    uiInit: function () {
      ...
      ui.screenSize.onResize();
      ui.screenSize.onOrientationChange();
      ui.screenSize.checkOrientation();
      ui.screenSize.scaleAndCenter();

      //awake elements
      ui.core.accordion.awake();
      ui.core.modal.awake();
      ui.core.menu.awake();
      ui.core.multi.awake();
      ui.core.tab.awake();
      ui.core.navigationArrows.awake();
      ui.core.link.awake();
      ui.core.relatedCLMLink.awake();
      ui.core.websiteLink.awake();

      //utilities menu
      ui.core.utilitiesMenu.awake();

      //standard or standalone functionality
      if (!this.vars.navigation.currentSlide.isStandalone) {
        //set arrows visibility (depending on slide position)
        ui.core.navigationArrows.setArrowsVisibility();
        //restore or reset session properties if back from standalone modal
        if (this.isBackFromStandAloneSlide()) {
          ui.common.backFromStandalone.restoreSessionStylesAndClasses();
        } else {
          ui.common.backFromStandalone.resetSessionPropertiesStylesAndClasses();
        }
      } else {
      ...
      if (/iPad/.test(navigator.userAgent)) {
        //iPad
        ui.common.swipeListener(document, "iPad", this.handleSwipeNavigation);
      } else {
        //other
        ui.common.swipeListener(document, "other", this.handleSwipeNavigation);
      }
      ...
      if (this.vars.references.active) {
        ui.core.references.init();
      }

      //disable interaction summary button if necessary
      if (!this.vars.interactionSummary.active) {
        ui.interactionSummary.disableOpenButton();
      }
      ...
      if (this.sessionData.complexLinks.element.id) {
        ...
        if (isPrevSlide || isPrevSlideMinus1) {
          ...
          if (this.sessionData.complexLinks.toSlide == this.vars.navigation.currentSlide.id) {
            ui.common.setComplexLinkElement();
            //clear data
    ...
    launchApprovedEmail: function (selectedOnly) {
      ...
      if (!vars.session.isAnActualCall) {
        util.log("clm.launchApprovedEmail: not an actual call", "error");
        return;
      }
  ...
  const util = {
    log: (pText, pType) => {
      let debugMode = clm.vars.options.debugMode.active;
      ...
      if (debugMode && ui.inspector.isActive()) ui.inspector.addLog(pText);
    },
    ...
    executeParameterFunction: (pFunction, pParam) => {
      if (pFunction !== "undefined" && pFunction !== "" && typeof window[pFunction] === "function") {
        ...
        util.log(`util.executeParameterFunction ERROR: ${pFunction} is not a function`, "error");
      }
    },
    ...
    readSetting: (pSettingsObj, pKeyChain, pType, pDefault, pLog) => {
      ...
      try {
        ...
        if (typeof pLog == "undefined" || pLog == true) {
          util.log(`readSetting ${pKeyChain} error: ${err}`, "error");
        }
      }
      ...
      if (pType == "string" && value) {
        if (value.startsWith("{{") && value.endsWith("}}")) {
          let schemaRelatedValue = util.getSchemaRelatedVar(value);
    ...
    getSchemaRelatedVar: (pToken) => {
      ...
      if (arrItem) {
        ...
        util.log(`getSchemaRelatedVar: ${pToken} not found in schemaRelatedVars`, "error");
      }
      ...
      for (let item in document.getElementsByTagName("script")) {
        ...
        if (scriptTag.src) {
          if (scriptTag.src.indexOf("common/") >= 0) {
            if (!clm.vars.options.browserMode.active) {
              path = "common/";
            } else {
              path = "../common/";
            }
    ...
    getAdjustImagesPath: function (slideId) {
      ...
      if (!slideId || slideId == "") {
        util.log(`getAdjustImagesPath: slideId is empty`, "error");
        return path;
      }

      if (!vars.options.adjustImagesPath || vars.options.adjustImagesPath == "common") {
        path = util.getSharedResourcesPath() + "img/adjust/" + slideId + "/";
      }
    ...
    getObjectSizeBytes: function (obj) {
    ...
    mutex: (() => {
      //used for calling the Veeva library functions in a mutex way

      let tail = Promise.resolve(); // last operation in the chain

      function run(fn) {
        // chain the new operation onto the tail
        const p = tail.then(() => fn());
        // add a catch to prevent unhandled rejections from breaking the chain
        tail = p.catch(() => {});
        return p; // return the new operation's promise
      }
  ...
  const ui = {
    btnFeedback: {
      ...
      enableSoundFeedback: function () {
        this.audio = new Audio(util.getSharedResourcesPath() + "mp3/" + "click.mp3");
        this.audio.volume = 0.01;

        document.querySelectorAll('[data-btn-response="true"]').forEach((el) => {
          el.addEventListener("mousedown", () => {
            if (clm.vars.options.btnFeedback.sound) {
              this.audio.currentTime = 0;
              this.audio.play().catch((error) => {
                util.log(`ui.btnFeedback.enableSoundFeedback: error playing sound - ${error}`, "error");
              });
            }
          });
      ...
      disableSoundFeedback: function () {
        clm.vars.options.btnFeedback.sound = false;
      },
    },
    core: {
      accordion: {
        ...
        awake: function () {
          document.querySelectorAll(this.selector).forEach((el) => {
            ...
            if (this.isHTMLValid(el, toValidate)) {
              ...
              let el_id;
              if (el.id === "") {
                el_id = ui.common.generateUniqueId("com_idc_ui_core_accordion", this.collection);
                el.id = el_id;
              ...
              el.querySelectorAll(':scope > [data-type="com.idc.ui.core.button"][data-sub-type="com.idc.ui.core.accordion.button"]').forEach((button) => {
                const instance = {
                  ...
                  params: ui.common.readElementOptions(
                    button,
                    {
                      beforeOpen: null,
                      afterOpen: null,
              ...
              //params
              el.params = ui.common.readElementOptions(
                el,
                {
              ...
              el.components.instances.forEach((instance) => {
                ...
                instance.executeBeforeOpen = ui.common.executeFunction.beforeOpen;
                instance.executeAfterOpen = ui.common.executeFunction.afterOpen;
                instance.executeBeforeClose = ui.common.executeFunction.beforeClose;
                instance.executeAfterClose = ui.common.executeFunction.afterClose;
              });
              ...
              if (clm.isBackFromStandAloneSlide()) {
                let activeInstances = ui.common.backFromStandalone.getSessionProperty(clm.vars.navigation.currentSlide.id, el.id, "activeInstances");
        ...
        setInstance: function (pInstance, pActive) {
          this.components.instances.forEach((instance) => {
            if (instance.name === pInstance) {
              ...
              if (pActive) {
                ...
                ui.common.resetChildElements(instance.content.element);
              }
            } else {
          ...
          if (!this.codeGeneratedId) {
            ...
            ui.common.backFromStandalone.setSessionProperty(clm.vars.navigation.currentSlide.id, this.id, "activeInstances", "viewState", activeInstances);
          }
        ...
        isHTMLValid: function (pElement, pToValidate) {
          ...
          pToValidate.attributes.forEach((attribute) => {
            if (util.getElementAttribute(pElement, attribute) === "") {
          ...
          if (errorList !== "") util.log(`${util.getElementAttribute(pElement, "data-type")} ${pElement.id}: ${errorList}`, "error");
      ...
      link: {
        ...
        awake: function () {
          document.querySelectorAll(this.selector).forEach((el) => {
            ...
            if (this.isHTMLValid(el, toValidate)) {
              ...
              let el_id;
              if (el.id === "") {
                el_id = ui.common.generateUniqueId("com_idc_ui_core_link", this.collection);
                el.id = el_id;
                el.setAttribute("id", el_id);
                el.codeGeneratedId = true;
              } else {
              ...
              el.addEventListener("click", (evt) => {
                ...
                //target id
                let targetId = util.getElementAttribute(el, "data-target-id")
                  ? util.getElementAttribute(el, "data-target-id")
                  : util.getElementAttribute(el, "data-target-id-fnc")
                  ? util.executeParameterFunction(util.getElementAttribute(el, "data-target-id-fnc"))
                  : "";

                //override function
                let overrideFnc;
                if (clm.vars.options.linkOverride.active) {
                  let localOverrideFnc = util.getElementAttribute(el, "data-link-override-fnc");
                  let globalOverrideFnc = clm.vars.options.linkOverride.global;
                ...
                let standaloneGroup = util.getElementAttribute(el, "data-standalone-group");
                if (standaloneGroup) {
                  if (!overrideFnc) {
                    if (clm.validateStandaloneGroup(standaloneGroup, targetId)) {
                      clm.activateStandaloneGroup(standaloneGroup);
                    }
                  } else {
                    util.log("com.idc.ui.core.link: link eval function / unable to activate standalone group", "error");
                  }
                }
                ...
                if (el.params.elId) {
                  ...
                  if (targetId && targetId.endsWith("..")) {
                    let slideFound = clm.findSlide(targetId.replace("..", ""), true);
                    if (slideFound) {
                ...
                if (targetId != "" && !overrideFnc) {
                  clm.gotoSlide(targetId);
                }

                //override function
                if (overrideFnc) {
                  util.executeParameterFunction(overrideFnc, { targetId: targetId });
                }
              });
            }
          });
        ...
        isHTMLValid: function (pElement, pToValidate) {
          ...
          let targetId = util.getElementAttribute(pElement, "data-target-id");
          if (!targetId) {
            let targetIdFnc = util.getElementAttribute(pElement, "data-target-id-fnc");
            if (targetIdFnc) {
              targetId = util.executeParameterFunction(targetIdFnc);
            }
          }

          let targetIdEmptyFlag = util.getElementAttribute(pElement, "data-target-id-empty-flag");

          if (targetId) {
            let slideFound;
            //if target id ends with .. perform and approximate search
            if (targetId && targetId.endsWith("..")) {
              slideFound = clm.findSlide(targetId.replace("..", ""), true);
            } else {
              slideFound = clm.findSlide(targetId);
            }
            //validate
        ...
        flagNonWorkingLinks: function () {
          ...
          document.querySelectorAll(this.selector).forEach((el) => {
            ...
            if (!util.getElementAttribute(el, "data-target-id")) return; //do not proceed if no target id

            let targetId = util.getElementAttribute(el, "data-target-id");

            //if target id ends with .. perform an approximate search
            if (targetId && targetId.endsWith("..")) {
              targetId = clm.findSlide(targetId.replace("..", ""), true).id;
            }
        ...
        awake: function () {
          document.querySelectorAll(this.selector).forEach((el) => {
            ...
            if (this.isHTMLValid(el, toValidate)) {
              ...
              let el_id;
              if (el.id === "") {
                el_id = ui.common.generateUniqueId("com_idc_ui_core_menu", this.collection);
                el.id = el_id;
                el.setAttribute("id", el_id);
                el.codeGeneratedId = true;
              } else {
        ...
        setActiveInstance: function () {
          document.querySelectorAll(this.selector).forEach((el) => {
            el.querySelectorAll(`[data-sub-type="com.idc.ui.core.menu.button"]`).forEach((button) => {
              ...
              if (targetId && targetId.endsWith("..")) {
                correspondsToCurrentSlide = clm.vars.navigation.currentSlide.id.includes(targetId.replace("..", ""));
              } else {
                correspondsToCurrentSlide = clm.vars.navigation.currentSlide.id == targetId;
              }
      ...
      modal: {
        ...
        awake: function () {
          ...
          document.querySelectorAll(this.selector).forEach((el) => {
            ...
            if (this.isHTMLValid(el, toValidate)) {
              ...
              let el_id;
              if (el.id === "") {
                el_id = ui.common.generateUniqueId("com_idc_ui_core_modal", this.collection);
                el.id = el_id;
                el.setAttribute("id", el_id);
                el.codeGeneratedId = true;
              } else {
              ...
              if (clm.vars.navigation.currentSlide.isStandalone) {
                if (clm.findSlide(clm.vars.navigation.currentSlide.id).standaloneModal.modalId == el.id) {
              ...
              el.executeBeforeOpen = ui.common.executeFunction.beforeOpen;
              el.executeAfterOpen = ui.common.executeFunction.afterOpen;
              el.executeBeforeClose = ui.common.executeFunction.beforeClose;
              el.executeAfterClose = ui.common.executeFunction.afterClose;
              ...
              el.components.backModal = {
                element: util.jsonToHTML(com.idc.templates.core.modal.backModal, null, el, "before"),
              };
              ...
              setTimeout(() => {
                ...
                let treatStandaloneModalsAsMainSlides = clm.vars.navigation.dynamicPresentation.treatStandaloneModalsAsMainSlides;
              ...
              if (el.isStandalone) {
                ...
                if (el.params.standaloneGroup) {
                  ...
                  if (clm.validateStandaloneGroup(el.params.standaloneGroup, clm.vars.navigation.currentSlide.id)) {
                    clm.activateStandaloneGroup(el.params.standaloneGroup);
                  }
                }
        ...
        open: function () {
          ...
          if (this.type == "popUp" || this.type == "dropDown") {
            ui.core.modal.activeModalsStack.push(this.id);
          }
          ...
          setTimeout(() => {
            let zIndex = ui.core.modal.activeModalsStack.length * 10 + increment;
          ...
          if (clm.vars.utilitiesMenu.active) {
            if (this.type == "popUp" || this.type == "dropDown") {
              setTimeout(() => {
                ui.core.utilitiesMenu.updateGroupsContents();
                ui.core.utilitiesMenu.updateStyleAndZIndex();
          ...
          ui.core.modal.setOpenModalFlag();

          //set arrows visibility
          ui.core.navigationArrows.setArrowsVisibility();

          //refresh references
          if (clm.vars.references.active) {
            if (this.type == "popUp" || this.type == "dropDown") {
              ui.core.references.refresh();
            }
          }
        ...
        close: function () {
          ...
          if (!this.isStandalone) {
            ...
            if (this.type == "popUp" || this.type == "dropDown") {
              if (ui.core.modal.activeModalsStack.indexOf(this.id) >= 0) {
                ui.core.modal.activeModalsStack.splice(ui.core.modal.activeModalsStack.indexOf(this.id), 1);
              }
            }
            ...
            //reset child objects (revert to default state)
            ui.common.resetChildElements(this);
            ...
            if (typeof this.params.closeAction === "string") {
              if (this.params.closeAction == null || this.params.closeAction == "opener" || this.params.closeAction == "openerStrict") {
                ...
                if (this.params.closeAction.endsWith("..")) {
                  slideFound = clm.findSlide(this.params.closeAction.replace("..", ""), true);
                } else {
                  slideFound = clm.findSlide(this.params.closeAction);
                }
                if (slideFound) {
            ...
            if (this.params.elId) {
              ...
              if (destinationSlide && destinationSlide.endsWith("..")) {
                let slideFound = clm.findSlide(destinationSlide.replace("..", ""), true);
                if (slideFound) {
            ...
            //redirect
            clm.gotoSlide(destinationSlide);
          }
        },
        hide: function () {
          ...
          switch (this.type) {
            case "dropDown":
              ...
              el.animate(
                ...
                //clear z-index
                el.style.zIndex = ui.core.modal.activeModalsStack.length;
            ...
            default:
              ...
              //clear z-index
              this.style.zIndex = ui.core.modal.activeModalsStack.length;
              break;
          }
        },
        isHTMLValid: function (pElement, pToValidate) {
        ...
        standaloneModalGroups: {
          ...
          setArrowsAndSwipe: function (pElement) {
            let group = clm.vars.standaloneModalGroups.groups.find((group) => group.id == clm.sessionData.session.selectedStandaloneGroup);
            if (!group) return;
            ...
            if (position.order > 0) {
              ...
              if (prevArrow && prevArrow.element && group.visibility.arrows) {
                ...
                prevArrow.element.addEventListener("click", (evt) => {
                  clm.gotoSlide(prevSlide);
                });
              }

              //navigation owerwrite
              clm.navigationOverwrite("prev", prevSlide);
            }

            if (position.order < position.total - 1) {
              //next slide
              let nextSlide = clm.vars.standaloneModalGroups.groups.find((group) => group.id == clm.sessionData.session.selectedStandaloneGroup).slides[
                position.order + 1
              ];
              ...
              if (nextArrow && nextArrow.element && group.visibility.arrows) {
                ...
                nextArrow.element.addEventListener("click", (evt) => {
                  clm.gotoSlide(nextSlide);
                });
              }

              //navigation owerwrite
              clm.navigationOverwrite("next", nextSlide);
            }
          },
          setGroupElementsVisibility: function (pElement) {
            ...
            let indexOpenButtonId = clm.vars.standaloneModalGroups.indexModal.openButton;
          ...
          populateGroupSlides: function (pIndexModalId, pActiveGroupId, pEnableLinkFunctionality) {
            ...
            if (pIndexModalId) {
              ...
              indexModalId = clm.vars.standaloneModalGroups.indexModal.id;
            }
            ...
            activeGroup.slides.forEach((slideId) => {
              let slide = clm.findSlide(slideId);
          ...
          preventCloseIfMandatoryGroup: function (pElement) {
            ...
            if (closeButton) {
              ...
              if (vars.utilitiesMenu.active) {
                ...
                ui.core.utilitiesMenu.hideItemInGroup("rightGroup", closeButton.element.id);
              }
            }
        ...
        awake: function () {
          document.querySelectorAll(this.selector).forEach((el) => {
            ...
            if (this.isHTMLValid(el, toValidate)) {
              ...
              let el_id;
              if (el.id === "") {
                el_id = ui.common.generateUniqueId("com_idc_ui_core_multi", this.collection);
                el.id = el_id;
                el.setAttribute("id", el_id);
                el.codeGeneratedId = true;
              } else {
              ...
              //restore saved state if returned from standalone pop-up
              if (clm.isBackFromStandAloneSlide()) {
                let activeInstance = ui.common.backFromStandalone.getSessionProperty(clm.vars.navigation.currentSlide.id, el.id, "activeInstance");
        ...
        setInstance: function (pInstance) {
          ...
          this.components.instances.forEach((instance) => {
            if (instance.name === pInstance) {
              ...
              //reset child objects (revert to default state)
              ui.common.resetChildElements(instance.element);
            }
          });
      ...
      navigationArrows: {
        ...
        setArrowsVisibility: function () {
          ...
          let navVars = clm.vars.navigation;
          ...
          if (needToHideArrows) {
            ...
            if (this.prevArrow) {
              if (!clm.vars.navigation.currentSlide.isFirst) {
                this.prevArrow.setAttribute("data-view-state", "active");
                //adjust z-index if necessary
                if (isCurrentSlideStandalone) {
                  this.prevArrow.style.zIndex = ui.core.modal.activeModalsStack.length * 10 + 1;
                }
              }
            }

            if (this.nextArrow) {
              if (!clm.vars.navigation.currentSlide.isLast) {
                this.nextArrow.setAttribute("data-view-state", "active");
                //adjust z-index if necessary
                if (isCurrentSlideStandalone) {
                  this.nextArrow.style.zIndex = ui.core.modal.activeModalsStack.length * 10 + 1;
                }
              }
            }
          }
      ...
      references: {
        ...
        refresh: function () {
            ...
            let alternativeRef = this.slideRefsArr.find((ref) => {
              ...
              if (ref.uiRelated) {
                ...
                if (ref.uiRelated.modal) {
                  if (ui.core.modal.activeModalsStack.indexOf(ref.uiRelated.modal.id) >= 0) {
                ...
                if (ref.uiRelated.multi) {
                  ...
                  if (multi) {
                    //check if multi is visible
                    if (multi.offsetParent) {
          ...
          //set content
          this.removeContent();
          this.setContent(ui.core.references.switch.selected, ui.core.references.activeRefs);
        },
        setEvents: function () {
          let vars = clm.vars;
        ...
        removeContent: function () {
          ...
          document
            .querySelector(`#${vars.references.components.modal.id} [data-type="com.idc.ui.references.landscapeView"]`)
            .removeAttribute("data-view-state");
      ...
      tab: {
        ...
        awake: function () {
          document.querySelectorAll(this.selector).forEach((el) => {
            ...
            if (this.isHTMLValid(el, toValidate)) {
              ...
              let el_id;
              if (el.id === "") {
                el_id = ui.common.generateUniqueId("com_idc_ui_core_tab", this.collection);
                el.id = el_id;
                el.setAttribute("id", el_id);
                el.codeGeneratedId = true;
              } else {
              ...
              //restore saved state if returned from standalone pop-up
              if (
                clm.isBackFromStandAloneSlide() &&
                ui.common.backFromStandalone.getSessionProperty(clm.vars.navigation.currentSlide.id, el.id, "activeInstance") !== null &&
                ui.common.backFromStandalone.getSessionProperty(clm.vars.navigation.currentSlide.id, el.id, "activeInstance").value !== null
              ) {
                //show instances
                el.components.container.element.setAttribute("data-view-state", "active");

                //set instance
                el.setInstance(ui.common.backFromStandalone.getSessionProperty(clm.vars.navigation.currentSlide.id, el.id, "activeInstance").value);
              } else {
                //not back from standalone slide: reset to defaults
                el.resetToDefaults();
              }
        ...
        setInstance: function (pInstance) {
          ...
          if (!this.codeGeneratedId) {
            ...
            ui.common.backFromStandalone.setSessionProperty(clm.vars.navigation.currentSlide.id, this.id, "activeInstance", "viewState", activeInstanceName);
          }
      ...
      utilitiesMenu: {
        ...
        updateGroupsContents: function () {
          ...
          if (vars.navigation.currentSlide.isStandalone && vars.utilitiesMenu.sets.standaloneModal.appendCloseButtonToRightGroup) {
            let standaloneModalId = clm.findSlide(vars.navigation.currentSlide.id).standaloneModal.modalId;
        ...
        updateStyleAndZIndex: function () {
          ...
          let activeModalsStack = ui.core.modal.activeModalsStack;
      ...
      relatedCLMLink: {
        ...
        awake: function () {
          ...
          document.querySelectorAll(this.selector).forEach((el) => {
            ...
            if (this.isHTMLValid(el, toValidate)) {
              ...
              let el_id;
              if (el.id === "") {
                el_id = ui.common.generateUniqueId("com_idc_ui_core_relatedCLMLink", this.collection);
                el.id = el_id;
                el.setAttribute("id", el_id);
                el.codeGeneratedId = true;
              } else {
              ...
              //params
              el.params = ui.common.readElementOptions(el, {});
              ...
              el.addEventListener("click", (evt) => {
                ...
                //target CLM id
                let targetCLMId = util.getElementAttribute(el, "data-target-clm");
                ...
                //goto CLM
                if (targetCLMId != "") {
                  clm.gotoRelatedCLM(targetCLMId);
                }
              });
            }
          });
        ...
        isHTMLValid: function (pElement, pToValidate) {
          ...
          //target CLM id
          let targetCLMId = util.getElementAttribute(pElement, "data-target-clm");
          let relatedCLM = vars.relatedCLM.find((clm) => {
        ...
        updateItemsState() {
          ...
          this.collection.forEach((el_id) => {
            ...
            if (el) {
              let targetCLMId = util.getElementAttribute(el, "data-target-clm");
              let relatedCLM = clm.vars.relatedCLM.find((clm) => {
                return clm.id == targetCLMId;
              });
      ...
      websiteLink: {
        ...
        awake: function () {
          document.querySelectorAll(this.selector).forEach((el) => {
            ...
            if (this.isHTMLValid(el, toValidate)) {
              ...
              let el_id;
              if (el.id === "") {
                el_id = ui.common.generateUniqueId("com_idc_ui_core_websiteLink", this.collection);
                el.id = el_id;
                el.setAttribute("id", el_id);
                el.codeGeneratedId = true;
              } else {
              ...
              //set href
              let targetWebsiteId = util.getElementAttribute(el, "data-target-website");
              let website = clm.vars.websites.find((web) => {
        ...
        isHTMLValid: function (pElement, pToValidate) {
          ...
          //target Website id
          let targetWebsiteId = util.getElementAttribute(pElement, "data-target-website");
    ...
    common: {
      ...
      executeFunction: {
        beforeOpen: function () {
          if (this.params.beforeOpen !== null) {
            util.executeParameterFunction(this.params.beforeOpen);
          }
        },
        afterOpen: function () {
          if (this.params.afterOpen !== null) {
            setTimeout(() => {
              util.executeParameterFunction(this.params.afterOpen);
            }, 500);
          }
        },
        beforeClose: function () {
          if (this.params.beforeClose !== null) {
            util.executeParameterFunction(this.params.beforeClose);
          }
        },
        afterClose: function () {
          if (this.params.afterClose !== null) {
            setTimeout(() => {
              util.executeParameterFunction(this.params.afterClose);
            }, 500);
          }
        },
      },
      ...
      readElementOptions: function (pEl, pOptionsSchema, pAbbreviations) {
        const optionsTxt = util.getElementAttribute(pEl, "data-options");
          ...
          if (element) {
            ...
            if (styles) {
              ...
              for (let i = 0; i < stylesArr.length; i++) {
                ...
                if (styleItem.length == 2) {
                  ...
                  this.setSessionProperty(clm.vars.navigation.currentSlide.id, pElementId, styleName, "style", styleValue);
                }
              }
            }
            ...
            if (classes) {
              ...
              for (let i = 0; i < classesArr.length; i++) {
                ...
                this.setSessionProperty(clm.vars.navigation.currentSlide.id, pElementId, className, "class", className);
              }
            }
          }
        },
        restoreSessionStylesAndClasses: function () {
          const slideId = clm.vars.navigation.currentSlide.id;
          const backFromStandalone = clm.sessionData.backFromStandalone;
          for (let i = 0; i < backFromStandalone.length; i++) {
      ...
      setComplexLinkElement: function () {
        let slideId = clm.vars.navigation.currentSlide.id;

        if (slideId != clm.sessionData.complexLinks.toSlide) return;

        let element = document.querySelector(`#${clm.sessionData.complexLinks.element.id}`);
        if (!element) return;

        let instance;

        switch (clm.sessionData.complexLinks.element.type) {
          case "accordion":
            instance = clm.sessionData.complexLinks.element.instance;
            if (instance) {
    ...
    dynamicPresentation: {
      setMenu: function (slidesSequence) {
        ...
        if (menuMode == "auto") {
          ...
          slidesSequence.forEach((slideId) => {
            ...
            button.innerHTML = clm.findSlide(slideId).description;
            button.setAttribute("data-target-id", slideId);
            button.setAttribute("data-type", "com.idc.ui.core.link");
            if (slideId == clm.vars.navigation.currentSlide.id) {
          ...
          //hide standard
          let standardMenu = document.querySelector(`#${clm.vars.options.dynamicPresentation.source[source].menu.standard.id}`);
      ...
      setAlertPopup: function () {
        ...
        if (isAlertPopUpActive) {
          util.jsonToHTML(com.idc.templates.dynamicPresentation.alertPopUp, "com_idc_ui_nonworkinglink_popup", document.querySelector("body"), null);
          ui.core.modal.awake();
        }
      },
    ...
    emailCart: {
      ...
      setEvents: function () {
          ...
          if (sendButton) {
            sendButton.addEventListener("click", (event) => {
              if (event.currentTarget.getAttribute("data-state") == "active") {
                ...
                clm.launchApprovedEmail(true); //selected only
              }
            });
          }
        ...
        if (this.elements.emailButton) {
          this.elements.emailButton.addEventListener("click", (event) => {
            if (event.currentTarget.getAttribute("data-state") == "active") {
              clm.launchApprovedEmail(false); //all fragments
            }
          });
        }
      },
      resetTempSelectedItems: function () {
        ...
        ui.emailCart.tempState.selectedItems = [];
        vars.emailCart[vars.emailCart.mode].forEach((item) => {
          if (item.selected) {
            ui.emailCart.tempState.selectedItems.push(item.id);
          }
        });
      },
      beforeOpen: function () {
        //set temp selected items
        ui.emailCart.resetTempSelectedItems();

        //toggle buttons state
        ui.emailCart.toggleButtonsState();

        //toggle items state
        ui.emailCart.toggleItemsState();
      },
      afterClose: function () {
        ui.emailCart.resetTempSelectedItems();
        ui.emailCart.toggleItemsState();
    ...
    inspector: {
      ...
      init: function () {
        ...
        //show button
        if (clm.vars.options.debugMode.showInspector) {
        ...
        new Promise((resolve) => {
          ...
          this.dropDown.element.overwriteParameterFunction("beforeOpen", () => {
            ui.inspector.refreshVars();
            ui.inspector.refreshSessionData();
          });
        });
          ...
          window.addEventListener(
            ...
            function (e) {
              ...
              if (required.every((q) => quadrants.has(q))) {
                reset();
                ui.inspector.dropDown.element.open();
              }
            },
            true
          );
      ...
      refreshParams: function () {
        let vars = JSON.parse(JSON.stringify(clm.vars));
        let params = {};
      ...
      refreshVars: function () {
        let vars = JSON.parse(JSON.stringify(clm.vars)); //all but the ones selected for params
        delete vars.project;
        delete vars.options;
      ...
      isActive: function () {
        return ui.inspector.dropDown.element;
      },
    },
    mediaDetection: {
      init: function () {
    ...
    screenSize: {
      ...
      checkOrientation: function () {
        ...
        util.dispatchEvent("konaOrientationChange", {
    ...
    interactionSummary: {
      ...
      init: function () {
        ...
        //elements
        if (vars.interactionSummary.components.waitView.id) {
          this.elements.waitView = document.querySelector(`#${vars.interactionSummary.components.waitView.id}`);
        }
        ...
        if (vars.interactionSummary.components.modal.id) {
          ...
          if (!this.elements.modal) {
            util.log(`ui.interactionSummary.init(): modal not found`, "error");
            return;
          }
        }
        ...
        //set mode
        if (isAnActualCall) {
          this.mode = "accountView";
        } else {
          this.mode = "allAccountsView";
        }

        //set labels
        util.setLabels(ui.interactionSummary.elements.modal, vars.interactionSummary.labels.waitView);

        //account view
        if (this.mode == "accountView" && vars.interactionSummary.options.accountView.active) {
          //confirm the account has activity before initializing, otherwise disable open button
          if (!vars.interactionSummary.output.currentAccount.noActivity) {
            this.accountView.init();
            this.accountView.populate();
          } else {
            ui.interactionSummary.disableOpenButton();
          }
        }
        ...
        //all accounts view
        if (this.mode == "allAccountsView" && vars.interactionSummary.options.allAccountsView.active) {
          this.accountView.init();
          this.allAccountsView.init();
        }

        //before open function
        window["preCallAfterOpen"] = () => {
          this.afterOpen();
        };
      },
      ...
      setActiveView: function (viewName) {
        //clear previous active view
        if (this.elements.waitView) {
          this.elements.waitView.removeAttribute("data-view-state");
        }
        if (this.accountView.elements.mainView) {
          this.accountView.elements.mainView.removeAttribute("data-view-state");
        }
        if (this.allAccountsView.elements.mainView) {
          this.allAccountsView.elements.mainView.removeAttribute("data-view-state");
        }
        ...
        switch (viewName) {
          case "accountView":
            if (this.accountView.elements.mainView) {
              this.accountView.elements.mainView.setAttribute("data-view-state", "active");
            }
            break;
          case "allAccountsView":
            if (this.allAccountsView.elements.mainView) {
              this.allAccountsView.elements.mainView.setAttribute("data-view-state", "active");
            }
            break;
        }
      },
      afterOpen: function () {
        ...
        //account view: show
        if (com.idc.ui.interactionSummary.mode == "accountView") {
          com.idc.ui.interactionSummary.setActiveView("accountView");
        }

        //all accounts view: generate model, populate and show
        if (com.idc.ui.interactionSummary.mode == "allAccountsView") {
          //check if model has already been generated, otherwise generate
          if (!vars.interactionSummary.output.ready) {
            //generate model
            util.mutex.run(() => clm.interactionSummaryDataModel());
            ...
            const interval = setInterval(() => {
              if (vars.interactionSummary.output.ready) {
                clearInterval(interval);
                //populate
                com.idc.ui.interactionSummary.allAccountsView.populate();

                //set active view
                com.idc.ui.interactionSummary.setActiveView("allAccountsView");
              }
            }, 500);
          }
        }
      ...
      accountView: {
        ...
        init: function () {
          ...
          //main view
          this.elements.mainView = document.querySelector(`#preCall [data-ui-id="mainViewAccount"]`);
          if (!this.elements.mainView) return;

          //elements
          if (vars.interactionSummary.components.accountView.tab.id) {
            this.elements.tab = ui.interactionSummary.elements.modal.querySelector(`#${vars.interactionSummary.components.accountView.tab.id}`);
            let cover = ui.interactionSummary.elements.modal.querySelector(
              `[data-type="com.idc.ui.core.tab.cover"][data-target-id="${vars.interactionSummary.components.accountView.tab.id}"]`
            );
            if (cover) {
            ...
            if (this.elements.tab) {
              ...
              //relatedCLM: sort
              this.elements.selectors.relatedCLM.sortSelect = this.elements.tab.querySelector(
                `[data-ui-id="relatedCLM"] [data-ui-id="relatedCLM_sort_select"]`
              );
            }
          }

          //set labels
          util.setLabels(ui.interactionSummary.elements.modal, vars.interactionSummary.labels.accountView);
        ...
        setSelectorsEvents: function () {
          ...
          if (this.elements.selectors.previousInteractions.filters) {
            this.elements.selectors.previousInteractions.filters
              ...
              .forEach((option) => {
                ...
                let recordsCount = vars.interactionSummary.output.currentAccount.timeline.filter((interaction) => {
                  switch (view) {
                    case "all":
                      return true;
                    case "approvedEmail":
          ...
          if (this.elements.selectors.slides.filters) {
            this.elements.selectors.slides.filters.querySelectorAll(`[data-ui-id="slides"] [data-ui-type="filter"]`).forEach((option) => {
              ...
              //check if are there any records for the selected view
              let recordsCount = vars.interactionSummary.output.currentAccount.slides.filter((slide) => {
                switch (view) {
                  case "all":
                    return true;
                  case "discussed":
          ...
          if (this.elements.selectors.slides.launchButton) {
            this.elements.selectors.slides.launchButton.addEventListener("click", (event) => {
              let callflow = clm.vars.interactionSummary.nextBestContent.clm.dynamicCallflowName;
              clm.setCallflow(callflow);
              let activeSlidesSequence = clm.vars.navigation.actualSlidesSequence;
              if (activeSlidesSequence.length > 0) {
                clm.gotoSlide(activeSlidesSequence[0]);
              }
            });
          }
            ...
            Array.from(this.elements.selectors.slides.sortSelect.options).forEach((option) => {
              ...
              switch (sort) {
                ...
                case "mostRecentlyDiscussed":
                  if (!vars.interactionSummary.visibility.accountView.fields.slides.lastView) {
                    option.remove();
                    return;
                  }
                  break;
                case "moreTimeSpentOn":
                  if (!vars.interactionSummary.visibility.accountView.fields.slides.duration) {
                    option.remove();
                    return;
                  }
                  break;
            ...
            Array.from(this.elements.selectors.emails.sortSelect.options).forEach((option) => {
              ...
              switch (sort) {
                ...
                case "mostRecentlySent":
                  if (!vars.interactionSummary.visibility.accountView.fields.emails.lastTimeSent_date) {
                    option.remove();
                  }
                  break;
                case "moreOpens":
                  if (!vars.interactionSummary.visibility.accountView.fields.emails.allTimesSent_open) {
                    option.remove();
                  }
                  break;
                case "moreClicks":
                  if (!vars.interactionSummary.visibility.accountView.fields.emails.allTimesSent_click) {
                    option.remove();
                  }
                  break;
              }
          ...
          if (this.elements.selectors.relatedCLM.filters) {
            this.elements.selectors.relatedCLM.filters.querySelectorAll(`[data-ui-id="relatedCLM"] [data-ui-type="filter"]`).forEach((option) => {
              ...
              //check if are there any records for the selected view
              let recordsCount = vars.interactionSummary.output.currentAccount.relatedCLM.filter((relatedCLM) => {
                switch (view) {
                  case "all":
                    return true;
                  case "discussed":
            ...
            Array.from(this.elements.selectors.relatedCLM.sortSelect.options).forEach((option) => {
              ...
              switch (sort) {
                case "default":
                  break;
                case "mostRecentlyDiscussed":
                  if (!vars.interactionSummary.visibility.accountView.fields.relatedCLM.lastView) {
                    option.remove();
                    return;
                  }
                  break;
        ...
        setTabsVisibility: function () {
          let vars = clm.vars;
          let visibility = vars.interactionSummary.visibility.accountView;
        ...
        populate: function () {
          let visibility = clm.vars.interactionSummary.visibility.accountView;
        ...
        accountName_Populate: function () {
          let vars = clm.vars;
          let elements = ui.interactionSummary.accountView.elements;

          if (!vars.interactionSummary.output.currentAccount) return;

          if (ui.interactionSummary.elements.modal) {
            ui.interactionSummary.elements.modal.querySelectorAll(`[data-ui-id="hcpName"]`).forEach((element) => {
              let accountName = vars.interactionSummary.output.currentAccount.account.name;
              if (vars.interactionSummary.output.currentAccount.account.salutation) {
                accountName = `${vars.interactionSummary.output.currentAccount.account.salutation} ${accountName}`;
              }
        ...
        cover_populate: function () {
          ...
          //previous interactions
          let previousInteractionsSpan = elements.cover.querySelector('[data-ui-id="tabBtn_previousInteractions_summary"]');
          if (previousInteractionsSpan) {
            let interactionsCount = vars.interactionSummary.output.currentAccount.timeline.length;
            if (interactionsCount == 0) {
              previousInteractionsSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_Interactions_noData;
            } else {
              let mostRecentDate = vars.interactionSummary.output.currentAccount.timeline[0].date;
              if (interactionsCount == 1) {
                previousInteractionsSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_Interactions_oneInteration.replace(
                  "||date||",
                  formatDate(mostRecentDate)
                );
              } else {
                previousInteractionsSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_Interactions_multipleInterations.replace(
                  "||date||",
                  formatDate(mostRecentDate)
                );
              }
          ...
          //slides
          let slidesSpan = elements.cover.querySelector('[data-ui-id="tabBtn_slides_summary"]');
          if (slidesSpan) {
            let tmpArr = vars.interactionSummary.output.currentAccount.slides
            ...
            if (slidesCount == 0) {
              slidesSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_Slides_noData;
            } else {
              let mostRecentDate = tmpArr[0].mostRecentCall.date;
              if (slidesCount == 1) {
                slidesSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_Slides_oneSlide.replace("||date||", formatDate(mostRecentDate));
              } else {
                slidesSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_Slides_multipleSlides.replace("||date||", formatDate(mostRecentDate));
              }
            }
            //next best content
            if (vars.interactionSummary.nextBestContent.clm.active) {
              let nbcSpan = elements.cover.querySelector('[data-ui-id="tabBtn_slides_nextBestContent"]');
              if (nbcSpan) {
                nbcSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_Slides_nextBestContent;
                nbcSpan.setAttribute("data-view-state", "active");
              }
            }
          }

          //emails
          let emailsSpan = elements.cover.querySelector('[data-ui-id="tabBtn_emails_summary"]');
          if (emailsSpan) {
            let tmpArr = vars.interactionSummary.output.currentAccount.emails
            ...
            if (emailsCount == 0) {
              emailsSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_Email_noData;
            } else {
              let mostRecentDate = tmpArr[0].mostRecentSent.date;
              let mostRecentStatus =
                tmpArr[0].mostRecentSent.opens > 0
                  ? vars.interactionSummary.labels.accountView.summary_Email_status_opened
                  : vars.interactionSummary.labels.accountView.summary_Email_status_notOpened;
              if (emailsCount == 1) {
                emailsSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_Email_oneEmail
                  .replace("||date||", formatDate(mostRecentDate))
                  .replace("||status||", mostRecentStatus);
              } else {
                emailsSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_Email_multipleEmails
                  .replace("||date||", formatDate(mostRecentDate))
                  .replace("||status||", mostRecentStatus);
              }
            }
          ...
          //related CLM
          let relatedCLMSpan = elements.cover.querySelector('[data-ui-id="tabBtn_relatedCLM_summary"]');
          if (relatedCLMSpan) {
            let tmpArr = vars.interactionSummary.output.currentAccount.relatedCLM
            ...
            if (relatedCLMCount == 0) {
              relatedCLMSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_RelatedCLM_noData;
            } else {
              let mostRecentDate = tmpArr[0].mostRecentCall.date;
              ...
              if (relatedCLMCount == 1) {
                relatedCLMSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_RelatedCLM_oneView
                  .replace("||date||", formatDate(mostRecentDate))
                  .replace("||material||", mostRecentMaterial);
              } else {
                relatedCLMSpan.innerHTML = vars.interactionSummary.labels.accountView.summary_RelatedCLM_multipleViews
                  .replace("||date||", formatDate(mostRecentDate))
                  .replace("||material||", mostRecentMaterial);
              }
            }
        ...
        previousInteractions_Populate: function () {
          let vars = clm.vars;
          let elements = ui.interactionSummary.accountView.elements;
          let options = ui.interactionSummary.accountView.options;

          let fieldsVisibility = vars.interactionSummary.visibility.accountView.fields.previousInteractions;
          const minInteractions = vars.interactionSummary.options.accountView.minRows.previousInteractions;
          let rowClassBool = true;
          let subRowClassBool = true;

          if (!elements.tab) return;
          if (!vars.interactionSummary.output.currentAccount) return;
          if (vars.interactionSummary.output.currentAccount.noActivity) return;

          let container = elements.tab.querySelector('[data-ui-id="previousInteractionsTable"]');
          ...
          switch (options.previousInteractions.filter) {
            case "all":
              filteredInteractions = vars.interactionSummary.output.currentAccount.timeline;
              break;
            case "approvedEmail":
              filteredInteractions = vars.interactionSummary.output.currentAccount.timeline.filter((interaction) => {
                return interaction.type == "email";
              });
              break;
            case "inPerson":
        ...
        slides_Populate: function () {
          ...
          let fieldsVisibility = vars.interactionSummary.visibility.accountView.fields.slides;

          if (!elements.tab) return;
          if (!vars.interactionSummary.output.currentAccount) return;
          if (vars.interactionSummary.output.currentAccount.noActivity) return;

          let container = elements.tab.querySelector('[data-ui-id="slides_contents"]');
          ...
          //filter slides
          let filteredSlides;
          switch (options.slides.filter) {
            case "all":
              filteredSlides = vars.interactionSummary.output.currentAccount.slides;
              break;
            case "discussed":
              filteredSlides = vars.interactionSummary.output.currentAccount.slides.filter((slide) => {
                return slide.status == "discussed";
              });
              break;
            case "notDiscussed":
          ...
          //sort slides
          if (options.slides.filter == "recommended") {
          ...
          filteredSlides.forEach((slide) => {
            //name
            let nameRecord = vars.interactionSummary.output.contentItems.slides.find((record) => {
              return record.id == slide.id;
            });
            ...
            slideClone.classList.add(options.slides.filter);
            ...
            let title;
            if (nameRecord) {
              title = nameRecord.title;
            }
            slideClone.querySelector('[data-type="slides_content_name"] [data-ui-type="value"]').innerHTML = title;
        ...
        emails_Populate: function () {
          ...
          let fieldsVisibility = vars.interactionSummary.visibility.accountView.fields.emails;
          const minEmails = vars.interactionSummary.options.accountView.minRows.emails;
          let rowClassBool = true;
          let subRowClassBool = true;
          ...
          let container = elements.tab.querySelector('[data-ui-id="emailsTable"]');
          let tbody = container.querySelector("tbody");
          let templates = {
            approvedEmail: {
              row: container.querySelector('[data-ui-type="table-row-template"][data-ui-subtype="approvedEmail"]'),
          ...
          //sort
          let filteredEmails = vars.interactionSummary.output.currentAccount.emails;
          switch (options.emails.sort) {
            case "default":
          ...
          filteredEmails.forEach((email) => {
            //name and thumbnail
            let nameRecord = vars.interactionSummary.output.contentItems.emails.find((record) => {
              return record.id == email.id;
            });
            ...
            if (nameRecord) {
              title = nameRecord.title;
              thumbnail = nameRecord.thumbnail;
            }

            if (thumbnail && thumbnail != "") {
              row.querySelector('[data-ui-subtype="thumbnail"] img').src = util.getSharedResourcesPath() + `img/emailCart/thumbs/${thumbnail}`;
            } else {
              row.querySelector('[data-ui-subtype="thumbnail"] img').style.display = "none";
            }
            row.querySelector('[data-ui-subtype="name"]').innerHTML = title;
            ...
            if (email.mostRecentSent.date) {
              ...
              if (fieldsVisibility.lastTimeSent_date) {
                row.querySelector('[data-ui-subtype="day"]').innerHTML = vars.interactionSummary.labels.accountView.emails_value_notSent;
              } else {
                row.querySelector('[data-ui-subtype="day"]').innerHTML = "";
              }
              row.querySelector('[data-ui-subtype="time"]').innerHTML = "";
            ...
            email.fragments.forEach((fragment) => {
              ...
              let title;
              if (nameRecord && nameRecord.fragments) {
                let fragmentNameRecord = nameRecord.fragments.find((record) => {
                  return record.id == fragment.id;
                });
                if (fragmentNameRecord && fragmentNameRecord.title && fragmentNameRecord.title != "") {
                  title = fragmentNameRecord.title;
                }
              }

              subRowClone.querySelector('[data-column-id="email"] [data-ui-type="table-subrow-cell-value"]').innerHTML = title;
        ...
        emails_ExpandCollapse: function (pRow, pContainer) {
          ...
          options.emails.expandAll = false;
        },
        ...
        relatedCLM_Populate: function () {
          ...
          let fieldsVisibility = vars.interactionSummary.visibility.accountView.fields.relatedCLM;

          if (!elements.tab) return;
          if (!vars.interactionSummary.output.currentAccount) return;
          if (vars.interactionSummary.output.currentAccount.noActivity) return;

          let container = elements.tab.querySelector('[data-ui-id="relatedCLM_contents"]');
          ...
          switch (options.relatedCLM.filter) {
            case "all":
              filteredRelatedCLM = vars.interactionSummary.output.currentAccount.relatedCLM;
              break;
            case "discussed":
              filteredRelatedCLM = vars.interactionSummary.output.currentAccount.relatedCLM.filter((relatedCLM) => {
                return relatedCLM.status == "discussed";
              });
              break;
            case "notDiscussed":
          ...
          //sort relatedCLM
          switch (options.relatedCLM.sort) {
            case "default":
          ...
          filteredRelatedCLM.forEach((relatedCLM) => {
            //name and thumbnail
            let nameRecord = vars.interactionSummary.output.contentItems.relatedCLM.find((record) => {
              return record.id == relatedCLM.id;
            });
            ...
            //filter related class
            relatedCLMClone.classList.add(options.relatedCLM.filter);
            ...
            let thumb;
            if (nameRecord) {
              thumb = nameRecord.thumb;
            }
            relatedCLMClone
              .querySelector('[data-type="relatedCLM_content_thumbnail"] img')
              .setAttribute("src", util.getSharedResourcesPath() + `img/relatedCLM/${thumb}`);
            ...
            relatedCLMClone.querySelector('[data-type="relatedCLM_content_name"] [data-ui-type="value"]').innerHTML = name;
            ...
            if (relatedCLM.status == "discussed") {
              ...
              if (fieldsVisibility.totalViews) {
                relatedCLMClone.querySelector('[data-type="relatedCLM_content_totalViews"] [data-ui-type="value"]').innerHTML =
                  relatedCLM.overall.timesDisplayed;
              } else {
      ...
      allAccountsView: {
        ...
        init: function () {
          ...
          //main view
          this.elements.mainView = document.querySelector(`#preCall [data-ui-id="mainViewAllAccounts"]`);
          if (!this.elements.mainView) return;

          //elements
          if (vars.interactionSummary.components.allAccountsView.tab.id) {
            this.elements.tab = ui.interactionSummary.elements.modal.querySelector(`#${vars.interactionSummary.components.allAccountsView.tab.id}`);
          }
          this.elements.filterSelect = this.elements.mainView.querySelector('[data-ui-id="allAccountsPresentationsFilterSelect"]');

          //set labels
          util.setLabels(ui.interactionSummary.elements.modal, vars.interactionSummary.labels.allAccountsView);
        ...
        setSelectorsEvents: function () {
          ...
          //presentations > accounts activity header columns
          {
            let nameColumn = elements.mainView.querySelector('[data-ui-id="allAccountsPresentationsHeaderAccount"]');
            let callsColumn = elements.mainView.querySelector('[data-ui-id="allAccountsPresentationsHeaderCalls"]');
            let lastActivityColumn = elements.mainView.querySelector('[data-ui-id="allAccountsPresentationsHeaderLast"]');

            //name column
            nameColumn.addEventListener("click", () => {
              //set sort option
              if (options.presentations.sort === "name_AZ") {
                options.presentations.sort = "name_ZA";
              } else {
                options.presentations.sort = "name_AZ";
              }

              //re-populate accounts activity
              this.populatePresentationsActivity(options.presentations.selectedPresentation);
            });

            //calls column
            callsColumn.addEventListener("click", () => {
              //set sort option
              if (options.presentations.sort === "mostCalls") {
                options.presentations.sort = "leastCalls";
              } else {
                options.presentations.sort = "mostCalls";
              }
            ...
            lastActivityColumn.addEventListener("click", () => {
              //set sort option
              if (options.presentations.sort === "mostRecent") {
                options.presentations.sort = "leastRecent";
              } else {
                options.presentations.sort = "mostRecent";
              }
          ...
          //presentations > filter
          {
            elements.filterSelect.addEventListener("change", (event) => {
              //set filter option
              options.presentations.filter = event.currentTarget.value;
              //re-populate accounts activity
              this.populatePresentationsActivity(options.presentations.selectedPresentation);
            });
          }
        ...
        setTabsVisibility: function () {
          let vars = clm.vars;
          let visibility = vars.interactionSummary.visibility.allAccountsView;
          let tab = this.elements.tab;

          if (!tab) return;

          //hide tabs if necessary
          if (!visibility.tabs.presentations) {
            tab.setInstanceVisibility("allAccountsPresentations", "hidden");
          }
          if (!visibility.tabs.emails) {
            tab.setInstanceVisibility("allAccountsEmails", "hidden");
          }
          if (!visibility.tabs.accounts) {
            tab.setInstanceVisibility("allAccountsAccounts", "hidden");
          }
        },
        switchToAccountView: function (accountId) {
          let vars = clm.vars;

          //find account record
          let accountRecord = vars.interactionSummary.output.allAccounts.find((record) => {
            return record.account.id === accountId;
          });
          if (!accountRecord) return;

          //set as current account
          vars.interactionSummary.output.currentAccount = accountRecord;

          //populate account view
          ui.interactionSummary.accountView.populate();

          //switch to account view tab
          ui.interactionSummary.setActiveView("accountView");
        },
        populate: function () {
          let visibility = clm.vars.interactionSummary.visibility.allAccountsView;

          //presentations
          if (visibility.tabs.presentations) {
            this.populatePresentationsList();
          }
        },
        highlightSelected: function (relevantId, viewType) {
          let panel, allItems;

          // ------------------------------------------------------------
          // 1. Resolve which panel and item collection to work with
          // ------------------------------------------------------------
          switch (viewType) {
            case "presentations":
              panel = this.elements.mainView.querySelector('[data-ui-id="allAccountsPresentationsMaterialsPanel"]');
              allItems = panel.querySelectorAll('[data-type="allAccountsPresentationsMaterialItem"]');
              break;
          ...
          if (!panel || !allItems || !allItems.length) return;

          // ------------------------------------------------------------
          // 2. Find the clicked (target) item, if same, reset view
          // ------------------------------------------------------------
          let targetItem = null;

          allItems.forEach((item) => {
            if (item.getAttribute("data-presentation-id") === relevantId) {
              targetItem = item;
            }
          });

          //no target, deselect everything and exit
          if (!targetItem) {

            //unlock scroll
            let panel = this.elements.mainView.querySelector('[data-ui-id="allAccountsPresentationsMaterialsPanel"]');
            panel.setAttribute("data-view-state", "idle"); // unlock scroll, reset view

            //deselect all
            let allItems = panel.querySelectorAll('[data-type="allAccountsPresentationsMaterialItem"]');
            allItems.forEach((item) => {
              item.removeAttribute("data-view-state"); // return to idle state
            });
          ...
          // ------------------------------------------------------------
          // 3. Determine if the clicked item is already active
          // ------------------------------------------------------------
          const isCurrentlyActive = targetItem.getAttribute("data-view-state") === "active";

          // ------------------------------------------------------------
          // 4A. If clicking the same active item → unselect everything
          // ------------------------------------------------------------
          ...
          // ------------------------------------------------------------
          // 4B. If clicking a different item → set active / inactive states
          // ------------------------------------------------------------
          allItems.forEach((item) => {
            if (item === targetItem) {
              item.setAttribute("data-view-state", "active");
            } else {
              item.setAttribute("data-view-state", "inactive");
            }
          });

          // Lock the left panel scroll when an item is selected
          panel.setAttribute("data-view-state", "locked");
        },
        populatePresentationsList: function () {
          let vars = clm.vars;
          let elements = ui.interactionSummary.allAccountsView.elements;
          let labels = vars.interactionSummary.labels.allAccountsView;

          if (!elements.tab) return;

          let container = elements.tab.querySelector('[data-ui-id="allAccountsPresentationsMaterialsList"]');
          let materialTemplate = container.querySelector('[data-ui-id="allAccountsPresentationsMaterialItemTemplate"][data-template="true"]');

          //remove previous
          container.querySelectorAll('[data-type="allAccountsPresentationsMaterialItem"]').forEach((material) => {
            material.remove();
          });
          ...
          presentationsArray.push({
            id: com.idc.clm.vars.project.name,
            name: labels.thisPresentation,
            thumb: null, //will use first thumb of first slide
            accountsActivity: vars.interactionSummary.output.contentItems.thisPresentation.accountsActivity,
            thisPresentation: true,
          });

          //add related CLM presentations
          vars.interactionSummary.output.contentItems.relatedCLM.forEach((relatedCLM) => {
            presentationsArray.push({
              id: relatedCLM.id,
              name: relatedCLM.name,
              thumb: relatedCLM.thumb,
              accountsActivity: relatedCLM.accountsActivity,
            });
          });

          //populate
          presentationsArray.forEach((presentation) => {
            let row = materialTemplate.cloneNode(true);

            //attributes
            row.setAttribute("data-type", "allAccountsPresentationsMaterialItem");
            row.setAttribute("data-template", "false");
            row.setAttribute("data-presentation-id", presentation.id);
            if (presentation.thisPresentation) {
              row.setAttribute("data-thisPresentation", "true");
            }

            //thumbnail
            let thumbSrc = "";
            if (presentation.thumb && presentation.thumb != "") {
              thumbSrc = util.getSharedResourcesPath() + `img/relatedCLM/${presentation.thumb}`;
            } else {
              //use first slide thumb of current presentation
              if (presentation.thisPresentation) {
                let firstSlide = com.idc.clm.vars.slides[0];
                thumbSrc = util.getSharedResourcesPath() + `img/thumbnails/${firstSlide.id}.png`;
              }
            }
            row.querySelector('[data-ui-id="allAccountsPresentationsMaterialThumb"] img').setAttribute("src", thumbSrc);
            row.querySelector('[data-ui-id="allAccountsPresentationsMaterialThumb"] img').setAttribute("alt", presentation.name);

            //pre-title
            if (presentation.thisPresentation) {
              row.querySelector('[data-ui-id="allAccountsPresentationsMaterialPretitle"]').innerHTML = labels.allAccountsPresentations_thisPresentation;
            } else {
              row.querySelector('[data-ui-id="allAccountsPresentationsMaterialPretitle"]').innerHTML = labels.allAccountsPresentations_quicklink;
            }

            //title
            row.querySelector('[data-ui-id="allAccountsPresentationsMaterialTitle"]').innerHTML = presentation.name;

            //set event listener
            row.addEventListener("click", (event) => {
              let presentationId = event.currentTarget.getAttribute("data-presentation-id");

              if (this.options.presentations.selectedPresentation === presentationId) {
                //clicking the same presentation → unselect
                this.options.presentations.selectedPresentation = null;
              } else {
                //select new presentation
                this.options.presentations.selectedPresentation = presentationId;
              }

              //highlight selected
              this.highlightSelected(this.options.presentations.selectedPresentation, "presentations");

              //enable/disable select filter
              if (this.options.presentations.selectedPresentation) {
                elements.filterSelect.removeAttribute("disabled");
              } else {
                elements.filterSelect.setAttribute("disabled", true);
                elements.filterSelect.value = "default";
                this.options.presentations.filter = "default";
              }

              //populate presentations activity
              this.populatePresentationsActivity(this.options.presentations.selectedPresentation);
        ...
        populatePresentationsActivity: function (presentationId) {
          let vars = clm.vars;
          let elements = ui.interactionSummary.allAccountsView.elements;
          let options = ui.interactionSummary.allAccountsView.options;

          let fieldsVisibility = vars.interactionSummary.visibility.allAccountsView.fields.presentations;

          let panel = elements.mainView.querySelector('[data-ui-id="allAccountsPresentationsAccountsPanel"]');
          let container = elements.tab.querySelector('[data-ui-id="allAccountsPresentationsAccountsList"]');
          let accountTemplate = container.querySelector('[data-ui-id="allAccountsPresentationsAccountsRowTemplate"][data-template="true"]');

          //remove previous
          container.querySelectorAll('[data-type="allAccountsPresentationsAccountsRowItem"]').forEach((account) => {
            account.remove();
          });

          //no presentation selected → set panel to idle
          if (!presentationId) {
            //set panel to idle
            panel.setAttribute("data-view-state", "idle");

            //remove styling from header columns
            let headerColumns = this.elements.mainView.querySelectorAll('[data-type="allAccountsPresentationsHeaderColumn"]');
            headerColumns.forEach((column) => {
              column.removeAttribute("data-sort-state");
            });
          ...
          {
            //header field: calls
            if (!fieldsVisibility.calls) {
              //flag
              this.elements.mainView.setAttribute("data-presentation_calls-hidden", true);
              //header label
              this.elements.mainView.querySelector(`[data-ui-id="allAccountsPresentationsHeaderCalls"]`).innerHTML = "";
            }

            //header field: last activity
            if (!fieldsVisibility.lastActivity) {
              //flag
              this.elements.mainView.setAttribute("data-presentation_lastActivity-hidden", true);

              //header label
              this.elements.mainView.querySelector(`[data-ui-id="allAccountsPresentationsHeaderLast"]`).innerHTML = "";
            }
          }

          //find presentation activity data
          let presentationRecord;
          //find in related CLM first
          presentationRecord = vars.interactionSummary.output.contentItems.relatedCLM.find((relatedCLM) => {
            return relatedCLM.id == presentationId;
          });
          //if not found, assume this presentation
          if (!presentationRecord) {
            presentationRecord = vars.interactionSummary.output.contentItems.thisPresentation;
          }

          if (!presentationRecord) return;

          //filter accounts
          let filteredAccountsActivity = presentationRecord.accountsActivity.slice(); //clone array
          switch (options.presentations.filter) {
            case "default":
              break;
            case "visitingThisWeek":
              filteredAccountsActivity = filteredAccountsActivity.filter((accountRecord) => {
                return accountRecord.flags.visitingThisWeek;
              });
              break;
            case "sawThisMaterial":
              filteredAccountsActivity = filteredAccountsActivity.filter((accountRecord) => {
                return accountRecord.flags.sawThisMaterial;
              });
              break;
            case "didNotSeeThisMaterial":
              filteredAccountsActivity = filteredAccountsActivity.filter((accountRecord) => {
                return !accountRecord.flags.sawThisMaterial;
              });
              break;
          ...
          //sort accounts activity
          let sortedAccountActivity = filteredAccountsActivity.slice(); //clone array
          switch (options.presentations.sort) {
            case "default":
            case "name_AZ":
              sortedAccountActivity.sort((a, b) => {
            ...
            case "mostRecent":
              sortedAccountActivity.sort((a, b) => {
                let dateA = a.lastActivity ? new Date(a.lastActivity) : new Date(0);
                let dateB = b.lastActivity ? new Date(b.lastActivity) : new Date(0);
                return dateB - dateA;
              });
              break;
            case "leastRecent":
          ...
          //header column sort indicators
          {
            //name column
            let nameColumn = this.elements.mainView.querySelector('[data-ui-id="allAccountsPresentationsHeaderAccount"]');
            nameColumn.removeAttribute("data-sort-state");
            if (options.presentations.sort === "name_AZ") {
              nameColumn.setAttribute("data-sort-state", "asc");
            } else if (options.presentations.sort === "name_ZA") {
              nameColumn.setAttribute("data-sort-state", "desc");
            }

            //calls column
            let callsColumn = this.elements.mainView.querySelector('[data-ui-id="allAccountsPresentationsHeaderCalls"]');
            callsColumn.removeAttribute("data-sort-state");
            if (options.presentations.sort === "mostCalls") {
              callsColumn.setAttribute("data-sort-state", "desc");
            } else if (options.presentations.sort === "leastCalls") {
              callsColumn.setAttribute("data-sort-state", "asc");
            }

            //last activity column
            let lastActivityColumn = this.elements.mainView.querySelector('[data-ui-id="allAccountsPresentationsHeaderLast"]');
            lastActivityColumn.removeAttribute("data-sort-state");
            if (options.presentations.sort === "mostRecent") {
              lastActivityColumn.setAttribute("data-sort-state", "desc");
            } else if (options.presentations.sort === "leastRecent") {
              lastActivityColumn.setAttribute("data-sort-state", "asc");
            }
          }

          //populate
          sortedAccountActivity.forEach((accountActivity) => {
            let accountClone = accountTemplate.cloneNode(true);

            //attributes
            accountClone.setAttribute("data-type", "allAccountsPresentationsAccountsRowItem");
            accountClone.setAttribute("data-template", "false");
            accountClone.setAttribute("data-account-id", accountActivity.id);

            //account name
            accountClone.querySelector('[data-ui-id="allAccountsPresentationsAccountsRowAccount"]').innerHTML = accountActivity.name;

            //calls
            if (fieldsVisibility.calls) {
              accountClone.querySelector('[data-ui-id="allAccountsPresentationsAccountsRowCalls"]').innerHTML = accountActivity.calls;
            } else {
              accountClone.querySelector('[data-ui-id="allAccountsPresentationsAccountsRowCalls"]').innerHTML = "";
            }

            //last activity
            if (fieldsVisibility.lastActivity) {
              if (accountActivity.lastActivity) {
                let dateArr = accountActivity.lastActivity.split("-");
                let tempDate = new Date(dateArr[0], dateArr[1] - 1, dateArr[2]); //format Month, Day, Year
                let formattedDate = tempDate.toLocaleDateString("en-US", {
                  month: "short",
                  day: "numeric",
                ...
                accountClone.querySelector('[data-ui-id="allAccountsPresentationsAccountsRowLast"]').innerHTML = formattedDate;
              } else {
                accountClone.querySelector('[data-ui-id="allAccountsPresentationsAccountsRowLast"]').innerHTML = " -- ";
              }
            } else {
              accountClone.querySelector('[data-ui-id="allAccountsPresentationsAccountsRowLast"]').innerHTML = "";
            }

            //set event listener for more button
            accountClone.querySelector('[data-ui-id="allAccountsPresentationsAccountsRowMore"]').addEventListener("click", () => {
              //switch to account view
              this.switchToAccountView(accountActivity.id);
            });

            //add to container
            container.appendChild(accountClone);
          });

          //set panel view state
          panel.setAttribute("data-view-state", "filled");
        },
      },
    },
  };
  ...
  const ai = {
    ...
    /*context (generated at the time of initialization) -----*/
    context: null,
    ...
    /*main logic --------------------------------------------*/
    api: {
      refreshAIContext: function () {
        let context = "";

        // --- SLIDES SECTION ------------------------------------------------------
        context += "THIS PRESENTATION CONTAINS EXACTLY THE FOLLOWING SLIDES::\n";

        // Try to use metadata slides first, if available
        const hasMetadata = typeof com_idc_clm_metadata === "object" && com_idc_clm_metadata !== null;

        if (!hasMetadata) {
          util.log("ai.api.refreshAIContext(): no CLM metadata object found.", "error");
        }

        const metadataSlides = hasMetadata && Array.isArray(com_idc_clm_metadata.slideContents) ? com_idc_clm_metadata.slideContents : null;

        if (metadataSlides && metadataSlides.length > 0) {
          // Build a map from slide id to metadata for quick lookup if needed later
          const metadataSlideMap = {};
          metadataSlides.forEach(function (ms) {
            if (ms && ms.id) {
              metadataSlideMap[ms.id] = ms;
            }
          });

          // If clm.vars.slides is available, we can enrich with both sources
          const runtimeSlides = Array.isArray(clm.vars.slides) ? clm.vars.slides : null;

          metadataSlides.forEach(function (metaSlide) {
            const slideId = metaSlide.id || "UNKNOWN_ID";
            const metaContent = metaSlide.content || "";
            const isNew = metaSlide.isNew === true;
            const whatIsNew = metaSlide.whatIsNew || "";

            context += `Slide Id: ${slideId}\n`;
            context += `Slide Summary: ${metaContent}\n`;

            // If isNew is true, explicitly mark it as new
            if (isNew) {
              context += `Slide Status: NEW\n`;
              if (whatIsNew) {
                context += `What Is New: ${whatIsNew}\n`;
              }
            }

            // Optionally include the original CLM runtime description if available
            if (runtimeSlides) {
              const runtimeSlide = runtimeSlides.find(function (s) {
                return s.id === slideId;
              });
              if (runtimeSlide && runtimeSlide.description) {
                context += `Technical Description: ${runtimeSlide.description}\n`;
              }
            }

            context += "\n";
          });
        } else if (Array.isArray(clm.vars.slides) && clm.vars.slides.length > 0) {
          // Fallback: use only clm.vars.slides as in your original implementation
          clm.vars.slides.forEach(function (slide) {
            context += `Slide Id: ${slide.id}\n`;
            context += `Slide Description: ${slide.description}\n\n`;
          });
        } else {
          // If no slide data is available at all
          context += "No slide metadata is available for this presentation.\n\n";
        }

        //Tell which slide we are on
        const currentSlideId = com.idc.clm.vars.navigation.currentSlide.id;
        context += `CURRENT SLIDE IN VIEW: ${currentSlideId}\n\n`;

        // --- FUNCTIONALITIES SECTION --------------------------------------------
        if (hasMetadata && com_idc_clm_metadata.functionalities) {
          const funcs = com_idc_clm_metadata.functionalities;
          const funcKeys = Object.keys(funcs);

          if (funcKeys.length > 0) {
            context += "THIS PRESENTATION OFFERS THE FOLLOWING FUNCTIONALITIES::\n";

            funcKeys.forEach(function (key) {
              const fn = funcs[key];
              if (!fn) return;

              const fnName = fn.name || key;
              const fnDesc = fn.description || "";
              const isNew = fn.isNew === true;
              const whatIsNew = fn.whatIsNew || "";

              context += `Functionality Id: ${key}\n`;
              context += `Functionality Name: ${fnName}\n`;
              context += `Functionality Description: ${fnDesc}\n`;

              if (isNew) {
                context += `Functionality Status: NEW\n`;
                if (whatIsNew) {
                  context += `What Is New: ${whatIsNew}\n`;
                }
              }
        ...
        // --- INTERACTION SUMMARY (dynamic high-level summary) -----------------------
        try {
          const raw = com?.idc?.clm?.vars?.interactionSummary?.output;

          if (raw) {
            const parsed = typeof raw === "string" ? JSON.parse(raw) : raw;

            context += "\nINTERACTION SUMMARY FOR THIS ACCOUNT::\n";

            // 1) ACCOUNT INFO --------------------------------------------------------
            if (parsed.account) {
              /* Anonymize account info for privacy              
              const acc = parsed.account;
              context += `Account Name: ${acc.salutation || ""} ${acc.name || ""}\n`;
              context += `Account ID: ${acc.id || "Unknown"}\n\n`;
              */
              context += `Account Info: [HIDDEN FOR PRIVACY]\n\n`;
            }

            // 2) RECENT TIMELINE (only last 4–5 items) ------------------------------
            if (Array.isArray(parsed.timeline)) {
              context += "Recent Engagement Timeline:\n";

              parsed.timeline.slice(0, 5).forEach((ev, i) => {
                context += `• ${ev.type.toUpperCase()} on ${ev.date} at ${ev.time} ${ev.time_AMPM}\n`;

                if (ev.type === "call" && ev.call) {
                  context += `   Channel: ${ev.call.channel}\n`;
                  if (Array.isArray(ev.call.slides)) {
                    context += `   Slides shown: ${ev.call.slides
                      .slice(0, 6)
                      .map((s) => s.id)
                      .join(", ")}\n`;
                  }
                ...
                if (ev.type === "email" && ev.email) {
                  context += `   Email: ${ev.email.title} | Opens: ${ev.email.opens}, Clicks: ${ev.email.clicks}\n`;
                }
              });
            ...
            // 3) SLIDE PERFORMANCE SUMMARY -------------------------------------------
            if (Array.isArray(parsed.slides)) {
              context += "Slide Performance Overview:\n";

              parsed.slides.forEach((sl) => {
                context += `• ${sl.id} (${sl.title}) — shown ${sl.overall?.timesDisplayed || 0} times`;
                if (sl.overall?.reaction) {
                  const r = sl.overall.reaction;
                  context += ` | reactions: +${r.positive || 0} / ~${r.neutral || 0} / -${r.negative || 0}`;
                }
                if (sl.mostRecentCall) {
                  context += ` | last seen: ${sl.mostRecentCall.date}`;
                }
                context += "\n";
              });
            ...
            if (Array.isArray(parsed.emails)) {
              context += "Email Engagement Summary:\n";

              parsed.emails.forEach((email) => {
                context += `• ${email.title} — sent ${email.overall?.sent || 0} times, opened ${email.overall?.opens || 0} times`;
                if (email.mostRecentOpen?.date) {
                  context += ` | last open: ${email.mostRecentOpen.date}`;
                }
                context += "\n";
              });
            ...
            // 5) RELATED CLMS ---------------------------------------------------------
            if (Array.isArray(parsed.relatedCLM)) {
              context += "Available Related CLM Presentations:\n";
              parsed.relatedCLM.forEach((clm) => {
                context += `• ${clm.name} (${clm.status}) — shown ${clm.overall?.timesDisplayed || 0} times\n`;
              });
              context += "\n";
            }
          }
        } catch (err) {
          context += "INTERACTION SUMMARY COULD NOT BE PROCESSED.\n\n";
        }

        // Assign final context to ai context variable used by your chat pipeline
        ai.context = context;
      },
      ...
      sendQuestion: function () {
        ...
        const labels = clm.vars.ai.labels;
        let sessionData = clm.sessionData;
        ...
        if (!question) {
          ui.updateStatus(labels.writeQuestionFirst);
          ui.setLoading(false);
          return;
        }

        // Start loading
        ui.updateStatus(labels.sendingQuestion);
        ui.setLoading(true);

        // Generate a unique request ID
        const requestId = "req-" + Date.now();
        ai.state.currentRequestId = requestId;

        // Clear input box
        ui.clearInputBox();

        // Hide suggested questions
        ui.hideSuggestedQuestions();

        // Show the question in the chat area
        ui.showQuestion(question, requestId);

        // Add to session var
        ai.helpers.updateSessionChatText("user", question);
        ...
        const formData = new FormData();
        {
          ...
          const SEPARATOR = "|||SEP|||";
          const conversationArray = sessionData.ai.chat.text || [];
          const flatConversation = conversationArray
            .map((msg) => {
              // just in case, remove separator from content
              const safeText = (msg.content || "").replaceAll(SEPARATOR, "");
              const role = msg.role || "user";
              return `${role}::${safeText}`;
            })
            .join(SEPARATOR);

          formData.append("conversation", flatConversation);

          //date/time in local timezone as ISO format, e.g., "2024-06-25T14:23:30.123Z"
          let now = new Date();
          let timezoneOffsetMs = now.getTimezoneOffset() * 60000;
          let localTime = new Date(now.getTime() - timezoneOffsetMs);
          let localISOTime = localTime.toISOString().slice(0, -1); // remove the 'Z' since it's local time
          formData.append("now_iso", localISOTime);

          //client timezone and locale
          let tz = Intl.DateTimeFormat().resolvedOptions().timeZone;
          let locale = navigator.language || "en-US";
          formData.append("client_timezone", tz);
          formData.append("client_locale", locale);
        }

        // Send to Zapier webhook
        fetch(clm.vars.ai.config.zapierWebhookUrl, {
          method: "POST",
          body: formData,
        })
          .then(function (res) {
            if (!res.ok) {
              log("com.idc.ai.sendQuestion: webhook returned " + res.status, "error");
            }

            ui.updateStatus(labels.analizingData);
            ai.api.waitForResponse(requestId, Date.now());
          })
          .catch(function (error) {
            ui.updateStatus(labels.errorSendingQuestion);
            ui.setLoading(false);
          });
      },

      waitForResponse: function (requestId, startTimeMs) {
        let labels = clm.vars.ai.labels;
        const now = Date.now();

        //timeout
        if (now - startTimeMs > clm.vars.ai.config.maxWaitMs) {
          ai.ui.updateStatus(labels.responseTimeout);
          ai.ui.setLoading(false);
          ai.ui.showRetryOption(requestId);
          return;
        }

        //label update
        if (now - startTimeMs > clm.vars.ai.config.tooLongMS * 2) {
          ai.ui.updateStatus(labels.aFewMoreSeconds);
        } else {
          if (now - startTimeMs > clm.vars.ai.config.tooLongMS) {
            ai.ui.updateStatus(labels.stillThinking);
          }
        }

        //fetch
        const url = ai.helpers.buildS3ResponseUrl(requestId);
        fetch(url, { cache: "no-store" })
          .then(function (res) {
            if (!res.ok) {
              // Response not ready yet, keep waiting
              ai.state.pollTimeoutId = setTimeout(function () {
                ai.api.waitForResponse(requestId, startTimeMs);
              }, clm.vars.ai.config.pollIntervalMs);
              return null;
            }
          ...
          .then(function (data) {
            if (!data) {
              // Already scheduled another check
              return;
            }

            ai.api.processResponse(data);
            ai.ui.updateStatus("");
          })
          .catch(function (error) {
      ...
      processResponse: function (data) {
        ...
        const text = data.answer_text || data.answer || JSON.stringify(data) || labels.responseError;

        // Relevant items
        const relevantSlides = data.relevantSlides || [];
        const relevantFunctionalities = data.relevantFunctionalities || [];
        const relevantSequence = data.relevantSequence || null;

        // Show response in UI
        ai.ui.showResponse(text, relevantSlides, relevantFunctionalities, relevantSequence);

        // Add to session var
        ai.helpers.updateSessionChatText("assistant", text);
      },

      retryQuestion: function (failedRequestId) {
        const sessionData = clm.sessionData;
        if (!sessionData.ai || !sessionData.ai.chat || !Array.isArray(sessionData.ai.chat.text)) {
          return;
        }
        const lastUserMessage = sessionData.ai.chat.text
          .slice()
          .reverse()
          .find((msg) => msg.role === "user");
        if (lastUserMessage && lastUserMessage.content) {
          // Re-enter the last user message into the input box
          ai.ui.elements.input.value = lastUserMessage.content;

          // Remove the last user message from session chat to avoid duplication
          sessionData.ai.chat.text = sessionData.ai.chat.text.filter((msg) => msg !== lastUserMessage);

          // Remove the last question and any error message from the chat display
          ai.ui.removeLastQuestion();

          // Update button state
          ai.ui.updateButtonState();
    ...
    ui: {
      ...
      init: function () {
        ...
        els.input = document.querySelector('[data-type="com.idc.ai.question.input"]');
        els.button = document.querySelector('[data-type="com.idc.ai.question.sendButton"]');
        els.spinner = document.querySelector('[data-type="com.idc.ai.status.spinner"]');
        els.statusText = document.querySelector('[data-type="com.idc.ai.status.text"]');
        els.responseContainer = document.querySelector('[data-type="com.idc.ai.chat"]');
        els.suggestedContainer = document.querySelector('[data-type="com.idc.ai.suggestedQuestions"]');

        // If the basic elements are not present, nothing to do
        if (!els.input || !els.button) return;

        const ui = this;

        // Update button state when the user types
        els.input.addEventListener("input", function () {
          ui.updateButtonState();
        });

        // Click handler – no inline onclick in HTML
        ...
        // Load previous chat from session data
        let sessionData = clm.sessionData;
        let hasPreviousChat = false;

        if (sessionData.ai && sessionData.ai.chat && sessionData.ai.chat.html) {
          els.responseContainer.innerHTML = sessionData.ai.chat.html;
          els.responseContainer.scrollTop = els.responseContainer.scrollHeight;
          hasPreviousChat = sessionData.ai.chat.html.trim().length > 0;
        }

        // If there is no previous chat, show suggested questions
        if (!hasPreviousChat && els.suggestedContainer) {
          ai.ui.renderSuggestedQuestions();
        } else if (els.suggestedContainer) {
          // If there is already chat, hide suggested questions
          ai.ui.hideSuggestedQuestions();
        }

        // Initialize orb drag + click + close button
        ai.ui.orb.init();
      },

      renderSuggestedQuestions: function () {
        var els = this.elements;
        var container = els.suggestedContainer;
        var inputEl = els.input;
        var buttonEl = els.button;
        var suggestedQuestions = clm.vars.ai.suggestedQuestions;

        if (!container || !inputEl || !buttonEl) return;
        ...
        suggestedQuestions.forEach(function (q) {
          ...
          item.setAttribute("data-type", "com.idc.ai.suggestedQuestions.item");
          item.textContent = q;

          // On click: fill input, focus, and (optionally) send
      ...
      hideSuggestedQuestions: function () {
        var container = this.elements.suggestedContainer;
        if (!container) return;
        container.innerHTML = "";
      },
      ...
      updateButtonState: function () {
        const els = this.elements;
        const inputEl = els.input;
        const buttonEl = els.button;
        if (!inputEl || !buttonEl) return;

        const hasText = inputEl.value.trim().length > 0;
        const isLoading = ai.state.isLoading;

        // Enabled only if there is text and we are not loading
        buttonEl.disabled = !hasText || isLoading;
      },
      ...
      updateInputState: function () {
        ...
        if (ai.state.isLoading) {
          inputEl.disabled = ai.state.isLoading;
          inputEl.setAttribute("placeholder", "");
        } else {
          inputEl.disabled = ai.state.isLoading;
          inputEl.setAttribute("placeholder", labels.inputPlaceholder);
        }
      },
      ...
      setLoading: function (isLoading) {
        const els = this.elements;
        ai.state.isLoading = !!isLoading;

        if (els.spinner) {
          els.spinner.style.display = isLoading ? "inline-block" : "none";
        }

        // Recalculate button enabled/disabled
      ...
      showQuestion: function (text, questionId) {
        const container = this.elements.responseContainer;
        if (!container) return;

        // remove retry options if any
        ai.ui.removeRetryOptions();

        // build question element
        const q = document.createElement("div");
        q.setAttribute("data-type", "com.idc.ai.chat.question");
        q.setAttribute("data-question-id", questionId);
        q.innerHTML = util.escapeHtml(text);
      ...
      showResponse: function (text, relevantSlides, relevantFunctionalities, relevantSequence) {
        const container = this.elements.responseContainer;
        if (!container) return;

        // build response element
        const r = document.createElement("div");
        r.setAttribute("data-type", "com.idc.ai.chat.response");
        r.setAttribute("data-response-to", ai.state.currentRequestId);

        // append to container
        container.appendChild(r);

        //typewrite effect
        ai.ui.typeText(r, text, 20, function () {
          //relevant slides
          if (Array.isArray(relevantSlides) && relevantSlides.length > 0) {
            ai.ui.showResponseRelevantSlides(r, relevantSlides);
          }

          //relevant functionalities
          if (Array.isArray(relevantFunctionalities) && relevantFunctionalities.length > 0) {
            ai.ui.showRelevantFunctionalities(r, relevantFunctionalities);
          }

          //relevant sequence
          if (Array.isArray(relevantSequence) && relevantSequence.length > 0) {
            //ai.ui.showRelevantSequence(r, relevantSequence);
          }

          //slowly scroll to bottom
          ai.ui.scrollToBottom();

          //once finished
          ai.ui.setLoading(false);

          // update session html
          ai.helpers.updateSessionChatHTML(container.innerHTML);
        });
      },

      showResponseRelevantSlides: function (container, relevantSlides) {
        const labels = clm.vars.ai.labels;

        // container for relevant slides
        const relevantContainer = document.createElement("div");
        relevantContainer.setAttribute("data-type", "com.idc.ai.chat.relevantItems");

        //for each slide, show thumbnail and description
        relevantSlides.forEach((slideId) => {
          let slide = clm.vars.slides.find((s) => s.id === slideId);
          if (!slide) return; //skip if not found

          const slideEl = document.createElement("div");
          slideEl.setAttribute("data-type", "com.idc.ai.chat.relevantItem");
          slideEl.innerHTML = `
            <div data-type="com.idc.ai.chat.relevantItem.thumbnail">
              <img src="${util.getSharedResourcesPath()}img/thumbnails/${slide.id}.png" alt="Slide ${util.escapeHtml(slide.description)} Thumbnail" />
            </div>
            <div data-type="com.idc.ai.chat.relevantItem.info">
              <div data-type="com.idc.ai.chat.relevantItem.name">${util.escapeHtml(slide.id)}</div>
              <div data-type="com.idc.ai.chat.relevantItem.description">${util.escapeHtml(slide.description)}</div>
            </div>
          `;

          //add link to slide in CLM
          slideEl.addEventListener("click", function () {
            clm.gotoSlide(slide.id);
          });

          relevantContainer.appendChild(slideEl);
        });

        //title
        const titleEl = document.createElement("div");
        titleEl.setAttribute("data-type", "com.idc.ai.chat.relevantItems.title");
        titleEl.textContent = labels.relevantSlides;
        container.appendChild(titleEl);

        //append to main container
        container.appendChild(relevantContainer);

        // Attach collapsible behavior (collapsed by default)
        ai.ui.attachRelevantToggle(titleEl, relevantContainer);
      },

      showRelevantFunctionalities: function (container, relevantFunctionalities) {
        const labels = clm.vars.ai.labels;

        // check metadata
        if (typeof com_idc_clm_metadata !== "object" || com_idc_clm_metadata === null || typeof com_idc_clm_metadata.functionalities !== "object") {
          return;
        }

        // container for relevant functionalities
        const relevantContainer = document.createElement("div");
        relevantContainer.setAttribute("data-type", "com.idc.ai.chat.relevantItems");

        //for each functionality, show thumbnail and description
        relevantFunctionalities.forEach((functionalityId) => {
          let functionality = {
            id: null,
            name: "",
            description: "",
          ...
          if (com_idc_clm_metadata.functionalities[functionalityId] == null) return; //skip if not found

          functionality.id = functionalityId;
          functionality.name = com_idc_clm_metadata.functionalities[functionalityId].name;
          functionality.description = com_idc_clm_metadata.functionalities[functionalityId].description;
          functionality.thumb = com_idc_clm_metadata.functionalities[functionalityId].thumb;
          functionality.action = com_idc_clm_metadata.functionalities[functionalityId].action;

          const functionalityEl = document.createElement("div");
          functionalityEl.setAttribute("data-type", "com.idc.ai.chat.relevantItem");
          functionalityEl.setAttribute("data-functionality-id", functionality.id);
          functionalityEl.innerHTML = `
            <div data-type="com.idc.ai.chat.relevantItem.thumbnail">
              <img src="${util.getSharedResourcesPath()}img/ai/functionalities/${functionality.thumb}" alt="Functionality ${util.escapeHtml(
            functionality.description
          )} Thumbnail" />
            </div>
            <div data-type="com.idc.ai.chat.relevantItem.info">
              <div data-type="com.idc.ai.chat.relevantItem.name">${util.escapeHtml(functionality.name)}</div>
              <div data-type="com.idc.ai.chat.relevantItem.description">${util.escapeHtml(functionality.description)}</div>
            </div>
          `;

          //open functionality in CLM
          functionalityEl.addEventListener("click", function () {
            if (functionality.action) {
              functionality.action();
            }
          });

          relevantContainer.appendChild(functionalityEl);
        });

        //title
        const titleEl = document.createElement("div");
        titleEl.setAttribute("data-type", "com.idc.ai.chat.relevantItems.title");
        titleEl.textContent = labels.relevantFunctionalities;
        container.appendChild(titleEl);
      ...
      attachRelevantToggle: function (titleEl, listEl) {
        if (!titleEl || !listEl) return;

        // Initial collapsed state
        //listEl.style.maxHeight = "0px";
        listEl.style.overflow = "hidden";
        listEl.setAttribute("data-expanded", "false");

        // Optional: make the title look interactive
        titleEl.style.cursor = "pointer";
        titleEl.setAttribute("data-expanded", "false");

        // Smooth transition for expand/collapse
        listEl.style.transition = "max-height 0.25s ease";

        titleEl.addEventListener("click", function () {
          var isExpanded = listEl.getAttribute("data-expanded") === "true";
          ...
          if (isExpanded) {
            ...
            // Expand: use scrollHeight so it fits content
            //listEl.style.maxHeight = listEl.scrollHeight + "px";
            listEl.setAttribute("data-expanded", "true");
            titleEl.setAttribute("data-expanded", "true");
          }

          // scroll to bottom after a slight delay to allow for expansion
          setTimeout(function () {
            ai.ui.scrollToBottom();
          }, 300);
        });
      ...
      typeText: function (el, text, speedMs, onDone) {
        ai.state.isTyping = true;

        const chatEl = document.querySelector('[data-type="com.idc.ai.chat"]');
        let i = 0;
      ...
      scrollToBottom: function () {
        const chatEl = document.querySelector('[data-type="com.idc.ai.chat"]');
        if (chatEl) {
          const startScrollTop = chatEl.scrollTop;
          ...
          function scrollStep(timestamp) {
            ...
            chatEl.scrollTop = startScrollTop + (targetScrollTop - startScrollTop) * progress;
            if (progress < 1) {
      ...
      showRetryOption: function () {
        ...
        retryEl.setAttribute("data-type", "com.idc.ai.chat.retryButton");
        retryEl.textContent = labels.retryBtn;
        retryEl.style.cursor = "pointer";

        retryEl.addEventListener("click", function () {
          ai.ui.removeRetryOptions();
          ai.api.retryQuestion();
        });
      ...
      removeRetryOptions: function () {
        const container = this.elements.responseContainer;
        if (!container) return;

        const retryEls = container.querySelectorAll('[data-type="com.idc.ai.chat.retryButton"]');
        retryEls.forEach(function (el) {
          el.remove();
        });
      },
      ...
      removeLastQuestion: function () {
        ...
        const questionEls = container.querySelectorAll('[data-type="com.idc.ai.chat.question"]');
      ...
      orb: {
        ...
        init: function () {
          ...
          var orbModule = ai.ui.orb;
          var orb = document.querySelector('[data-type="com.idc.ai.orb"]');
          var closeBtn = document.querySelector('[data-type="com.idc.ai.close"]');
          var container = document.querySelector('[data-type="com.idc.ai.container"]');
          ...
          //set initial and current position
          orb.style.left = orbModule.initialPosition.left + "px";
          orb.style.top = orbModule.initialPosition.top + "px";
          orbModule.currentPosition.left = orbModule.initialPosition.left;
          orbModule.currentPosition.top = orbModule.initialPosition.top;
          let orbWidth = orb.offsetWidth;
          let orbHeight = orb.offsetHeight;

          //set data-view-state=active
          orb.setAttribute("data-view-state", "active");

          function applyAnchorToUi() {
            if (!closeBtn || !container) return;

            // Orb center based on your dragged position (currentPosition)
            var orbCenterX = orbModule.currentPosition.left + orbWidth / 2;
            var orbCenterY = orbModule.currentPosition.top + orbHeight / 2;

            // Close button size
            var closeRect = closeBtn.getBoundingClientRect();
            var closeSize = closeRect.width || 40; // fallback if not yet rendered
            ...
            var margin = 16; // minimum distance from panel to viewport edges

            // Decide which screen half the orb is in
            var isLeftSide = orbCenterX < viewportWidth / 2;
            var isTopSide = orbCenterY < viewportHeight / 2;

            var closeX, closeY;
            var panelLeft, panelTop;

            // ----------------------------------------------------------
            // 1) Position the close button so it's perfectly centered on the orb
            // ----------------------------------------------------------
            closeX = orbCenterX - closeSize / 2;
            closeY = orbCenterY - closeSize / 2;

            // ----------------------------------------------------------
            // 2) Position the chat container so that *one of its corners*
            //    matches the center of the close button (orb center),
            //    chosen dynamically by quadrant.
            //
            //    Quadrants:
            //    - Top-left:     use panel's top-left corner at close center
            //    - Bottom-left:  use panel's bottom-left corner at close center
            //    - Top-right:    use panel's top-right corner at close center
            //    - Bottom-right: use panel's bottom-right corner at close center
            // ----------------------------------------------------------

            if (isLeftSide && isTopSide) {
              // Orb is in top-left area: panel extends to the right and down
              // Top-left corner of panel at close center
              panelLeft = orbCenterX + 6;
              panelTop = orbCenterY + 5;
            } else if (isLeftSide && !isTopSide) {
              // Orb is in bottom-left area: panel extends to the right and up
              // Bottom-left corner of panel at close center
              panelLeft = orbCenterX + 4;
              panelTop = orbCenterY - panelHeight + 6;
            } else if (!isLeftSide && isTopSide) {
              // Orb is in top-right area: panel extends to the left and down
              // Top-right corner of panel at close center
              panelLeft = orbCenterX - panelWidth + 5;
              panelTop = orbCenterY + 6;
            } else {
              // Orb is in bottom-right area: panel extends to the left and up
              // Bottom-right corner of panel at close center
              panelLeft = orbCenterX - panelWidth + 4;
              panelTop = orbCenterY - panelHeight + 6;
            }

            // ----------------------------------------------------------
            // 3) Clamp panel so it stays inside the viewport
            // ----------------------------------------------------------
            panelLeft = Math.max(margin, Math.min(panelLeft, viewportWidth - panelWidth - margin));
            panelTop = Math.max(margin, Math.min(panelTop, viewportHeight - panelHeight - margin));

            // ----------------------------------------------------------
            // 4) Apply close button position
            // ----------------------------------------------------------
            closeBtn.style.position = "fixed";
            closeBtn.style.left = closeX + "px";
            closeBtn.style.top = closeY + "px";
            closeBtn.style.right = "auto";
            closeBtn.style.bottom = "auto";

            // ----------------------------------------------------------
            // 5) Apply chat container position
            // ----------------------------------------------------------
          ...
          function startDrag(e) {
            if (body.getAttribute("data-ai-open") === "true") return;
          ...
          function drag(e) {
            ...
            var deltaX = evt.clientX - dragStartX;
            var deltaY = evt.clientY - dragStartY;

            var maxLeft = window.innerWidth - orbWidth / 2;
            var maxTop = window.innerHeight - orbHeight / 2;
            var minLeft = orbWidth / 2;
            var minTop = orbHeight / 2;

            // Constrain within viewport
            if (orbModule.initialPosition.left + deltaX < minLeft) {
              deltaX = minLeft - orbModule.initialPosition.left;
            } else if (orbModule.initialPosition.left + deltaX > maxLeft) {
              deltaX = maxLeft - orbModule.initialPosition.left;
            }
            if (orbModule.initialPosition.top + deltaY < minTop) {
              deltaY = minTop - orbModule.initialPosition.top;
            } else if (orbModule.initialPosition.top + deltaY > maxTop) {
              deltaY = maxTop - orbModule.initialPosition.top;
            }

            if (!isDragging && (Math.abs(deltaX) > 0 || Math.abs(deltaY) > 0)) {
              isDragging = true;
            ...
            if (isDragging) {
              ...
              orbModule.currentPosition.top = orbModule.initialPosition.top + deltaY;
              orbModule.currentPosition.left = orbModule.initialPosition.left + deltaX;

              orb.style.top = orbModule.currentPosition.top + "px";
              orb.style.left = orbModule.currentPosition.left + "px";
            }
          }
          ...
          function endDrag() {
            ...
            orbModule.initialPosition.top = orbModule.currentPosition.top;
            orbModule.initialPosition.left = orbModule.currentPosition.left;
          ...
          function openChat() {
            ...
            ai.api.refreshAIContext();
            body.setAttribute("data-ai-open", "true");
          }

          function closeChat() {
            const body = document.body;
            body.setAttribute("data-ai-open", "false");
          }

          // Orb events
          orb.addEventListener("mousedown", startDrag);
          orb.addEventListener("touchstart", startDrag, { passive: true });
    ...
    helpers: {
      ...
      buildS3ResponseUrl: function (requestId) {
        const now = new Date();
        ...
        return clm.vars.ai.config.s3BaseUrl + yyyy + "/" + mm + "/" + dd + "/" + encodeURIComponent(safeId) + ".json";
      },

      updateSessionChatText: function (role, content) {
        let sessionData = clm.sessionData;
        sessionData.ai.chat.text.push({
          role: role,
          content: content,
        });
        clm.updateSessionData();
      ...
      updateSessionChatHTML: function (htmlContent) {
        let sessionData = clm.sessionData;

        sessionData.ai.chat.html = htmlContent;
        clm.updateSessionData();
      },
    },
  };
  ...
  global.pageUnload = function () {};
  if (global.com && global.com.veeva && global.com.veeva.clm) {
    global.com.veeva.clm.createRecordsOnExit = function () {};
    global.com.veeva.clm.updateRecordsOnExit = function () {};
    global.com.veeva.clm.updateCurrentRecordsOnExit = function () {};
  }
})(window);
