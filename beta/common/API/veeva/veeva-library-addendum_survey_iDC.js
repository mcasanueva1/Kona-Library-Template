// Veeva JavaScript Library Survey Addendum v1.1
                                            ...
                                            com.veeva.clm.getDataForObject(
                                                ...
                                                function(dataReceived) {
                                                    if (dataReceived.success) {
                                                        ...
                                                        returnError('com.veeva.clm.getSurvey_Object: Unable to get Survey DeveloperName. Code: ' + dataReceived.code + '. Message:' + dataReceived.message);
                                                    }
                                                });
                                        } else {
                                            returnError('com.veeva.clm.getSurvey_Object: Unable to get Survey RecordType ID. Code: ' + dataReceived.code + '. Message:' + dataReceived.message);
                                        }
                                    });
                            } else {
                        ...
                        com.veeva.clm.getDataForObject(
                            ...
                            function(dataReceived) {
                                if (dataReceived.success) {
                                    ...
                                    returnError('com.veeva.clm.getSurvey_Object: Unable to get Survey Target status. Code: ' + dataReceived.code + '. Message:' + dataReceived.message);
                                }
                            });
                            ...
                            function(dataReceived) {
                                if (dataReceived.success) {
                                    ...
                                    com.veeva.clm.getDataForObject(
                                        ...
                                        function(dataReceived) {
                                            if (dataReceived.success) {
                                                questions[fetchPosition].Required_vod__c = dataReceived.Survey_Question_vod__c.Required_vod__c;
                                                //type
                                                com.veeva.clm.getDataForObject(
                                                    'Survey_Question_vod__c',
                                                    questions[fetchPosition].ID,
...
com.veeva.clm.submitSurvey = function(surveyObject, action, onRequestSuccess, onRequestError) {
    ...
    var questionResponseRecordTypesArray = [];
    var surveyRecordTypesArray = [];
    var surveyTargetRecordTypesArray = [];
    ...
    var startSubmitSequence = function() {
        if ((action != 'Submitted_vod') && (action != 'Saved_vod')) {
            ...
            getSurveyRecordTypeIds();
        }
    }

    //find CLM survey and survey target recordtypes
    var getSurveyRecordTypeIds = function() {
        com.veeva.clm.getRecordType_Object(
            'Survey_vod__c',
            function(dataReceived) {
                if (dataReceived.success) {
                    surveyRecordTypesArray = [];
                    for (var v = 0; v < dataReceived.RecordType.length; v++) {
                        var recordTypeObj = {};
                        recordTypeObj.id = dataReceived.RecordType[v].ID
                        surveyRecordTypesArray.push(recordTypeObj);
                    }
                    fetchPosition = 0;
                    getSurveyRecordTypeDeveloperNames();
                } else {
                    returnError('com.veeva.clm.submitSurvey: Unable to get Survey recordtypes. Code: ' + dataReceived.code + '. Message:' + dataReceived.message);
                }
            });
    }

    var getSurveyRecordTypeDeveloperNames = function() {
        com.veeva.clm.getDataForObject(
            'RecordType',
            surveyRecordTypesArray[fetchPosition].id,
            'DeveloperName',
            function(dataReceived) {
                if (dataReceived.success) {
                    surveyRecordTypesArray[fetchPosition].developerName = dataReceived.RecordType.DeveloperName;
                    if (fetchPosition < surveyRecordTypesArray.length - 1) {
                        fetchPosition++;
                        getSurveyRecordTypeDeveloperNames();
                    } else {
                        fetchPosition = 0;
                        getSurveyTargetRecordTypeIds();
                    }
                } else {
                    returnError('com.veeva.clm.submitSurvey: Unable to get Survey recordtypes developer name. Code: ' + dataReceived.code + '. Message:' + dataReceived.message);
                }
            });
    }

    var getSurveyTargetRecordTypeIds = function() {
        com.veeva.clm.getRecordType_Object(
            'Survey_Target_vod__c',
            function(dataReceived) {
                if (dataReceived.success) {
                    surveyTargetRecordTypesArray = [];
                    for (var v = 0; v < dataReceived.RecordType.length; v++) {
                        var recordTypeObj = {};
                        recordTypeObj.id = dataReceived.RecordType[v].ID
                        surveyTargetRecordTypesArray.push(recordTypeObj);
                    }
                    fetchPosition = 0;
                    getSurveyTargetRecordTypeDeveloperNames();
                } else {
                    returnError('com.veeva.clm.submitSurvey: Unable to get Survey Target recordtypes. Code: ' + dataReceived.code + '. Message:' + dataReceived.message);
                }
            });
    }

    var getSurveyTargetRecordTypeDeveloperNames = function() {
        com.veeva.clm.getDataForObject(
            'RecordType',
            surveyTargetRecordTypesArray[fetchPosition].id,
            'DeveloperName',
            function(dataReceived) {
                if (dataReceived.success) {
                    surveyTargetRecordTypesArray[fetchPosition].developerName = dataReceived.RecordType.DeveloperName;
                    if (fetchPosition < surveyTargetRecordTypesArray.length - 1) {
                        fetchPosition++;
                        getSurveyTargetRecordTypeDeveloperNames();
                    } else {
                        fetchPosition = 0;
                    ...
                    returnError('com.veeva.clm.submitSurvey: Unable to get Survey Target recordtypes developer name. Code: ' + dataReceived.code + '. Message:' + dataReceived.message);
                }
            });
    }
    ...
    var getResponseRecordTypes = function() {
        com.veeva.clm.getRecordType_Object(
            ...
            function(dataReceived) {
                if (dataReceived.success) {
                    questionResponseRecordTypesArray = [];
                    for (var v = 0; v < dataReceived.RecordType.length; v++) {
                        questionResponseRecordTypesArray.push(dataReceived.RecordType[v].ID);
                    }
                    fetchPosition = 0;
                    findCLMResponseRecordTypeId();
                } else {
                    returnError('com.veeva.clm.submitSurvey: Unable to get Question response recordtype IDs. Code: ' + dataReceived.code + '. Message:' + dataReceived.message);
                }
            });
    }
    ...
    var findCLMResponseRecordTypeId = function() {
        com.veeva.clm.getDataForObject(
            'RecordType',
            questionResponseRecordTypesArray[fetchPosition],
            'DeveloperName',
            function(dataReceived) {
                if (dataReceived.success) {
                    if (dataReceived.RecordType.DeveloperName == 'CLM_vod') {
                        clmResponseRecordTypeId = questionResponseRecordTypesArray[fetchPosition];
                        fetchPosition = 0;
                        evaluateSurveyStatus();
                    } else {
                        if (fetchPosition < questionResponseRecordTypesArray.length - 1) {
                            fetchPosition++;
                    ...
                    returnError('com.veeva.clm.submitSurvey: Unable to get CLM RecordType developer name. Code: ' + dataReceived.code + '. Message:' + dataReceived.message);
                }
            });
    }
    ...
    var createSurveyTarget = function() {
        ...
        function findSurveyTargetRecordType(surveyRecordTypeId) {
            var tmpDeveloperName = '';
            var equivalentRecordType = '';

            for (var i = 0; i < surveyRecordTypesArray.length; i++) {
                if (surveyRecordTypesArray[i].id == surveyRecordTypeId) {
                    tmpDeveloperName = surveyRecordTypesArray[i].developerName
                }
            }

            if (tmpDeveloperName != '') {
                for (var i = 0; i < surveyTargetRecordTypesArray.length; i++) {
                    if (surveyTargetRecordTypesArray[i].developerName == tmpDeveloperName) {
                        equivalentRecordType = surveyTargetRecordTypesArray[i].id;
                    }
                }
            }
            ...
            function(dataReceived) {
                if (dataReceived.success) {
                    ...
                    com.veeva.clm.getDataForObject(
                        ...
                        function(dataReceived) {
                            if (dataReceived.success) {
                                var surveyTargetEquivalentRecordtype = findSurveyTargetRecordType(dataReceived.Survey_vod__c.RecordTypeId)
                                if (surveyTargetEquivalentRecordtype != '') {
                                    surveyTargetObject.RecordTypeId = surveyTargetEquivalentRecordtype;
                                    //territory
                                    com.veeva.clm.getDataForObject(
                                        'Survey_vod__c',
                                        surveyObject.ID,
                                    ...
                                    returnError('com.veeva.clm.submitSurvey: Unable to get Survey Target equivalent RecordTypeId');
                                }
                            } else {
